...
- **Pages** — Owners publish a static site with **Push Manifest** (required). A README pagelink and custom site name are optional. The `/pages` directory shows a first page of sites, then Load more — same two-column cards as `/apps` (icon, hostname, author avatar / nip05 / TrustBadge, Named vs Npub-site pill, paths / snapshots / hits). The author name opens that person’s gittr profile in a new tab.
