...
- **Apps** — Browse NIP-82 / Zapstore apps at [/apps](https://gittr.space/apps). Owners **Announce** a forge Release tag from the Code sidebar (**Nostr Apps**, latest) or the Releases tab (same form, chosen tag). That lists the app; it does **not** add the APK to the git tree. Visitors get **Links → App (id)** on the Code About sidebar (hydrated from the catalog if this browser never announced) and downloads on **Releases**. Duplicate App rows for the same package id are collapsed. Signed-in: user menu **Your Apps** lists your cards; **Remove listing** deletes one app id (the leftover `GITTR` card, for example) without deleting the git repo or the APK file. The Code-tab App (id) link stays so the next Push still publishes it. Details: [Help → Releases](https://gittr.space/help#releases).
