...
- **Issues & PRs on Nostr** — Same workflow as a normal forge. A PR event is the title, description, and git pointers; the patch lives on the clone host (ngit, gitworkshop, and gittr all read those pointers). Comments on issues **and** PRs are NIP-22 (kind **1111**) on that event.
