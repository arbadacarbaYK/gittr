...
| Per-repo title / description / OG image | `ui/src/app/[entity]/[repo]/layout.tsx` + `opengraph-image.tsx` | Composed **1200×630** dark card: name, owner, About, corner badge; stats as **GitHub icon + ★ count**, fork icon + count, **N + ★** for Nostr (no “source stars” prose). Brand bottom-right (X uses bottom-left). Vanity paths (`/DrShift/buho-go`) **307** to `/{npub}/buho-go` (issues/commits suffix + query kept) so Code, files, Push, and refetch use a real owner pubkey. Crawlers that follow that redirect load the npub card. The vanity URL still composes `opengraph-image` (no redirect on the image route) and `og:url` / canonical already point at the npub path. Bare `/DrShift` profile URLs are not rewritten. |
