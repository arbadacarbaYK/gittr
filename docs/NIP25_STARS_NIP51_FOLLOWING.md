...
- **Star** needs a resolved kind **30617** event id. That id (and `clone[]` / `source` / `public-read`) comes from **`resolveLiveRepoAnnouncement`** — the same `/api/nostr/profile-repos` path file fetch already uses — plus a live subscribe overlay. Do not treat “files loaded from the owner’s GRASP host” as unpublished. `d` tag match is case-insensitive. Watch uses `a` tag `30617:owner:d` (not the event id).
