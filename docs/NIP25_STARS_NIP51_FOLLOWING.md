...
- **Star** needs a resolved kind **30617** event id. That id (and `clone[]` / `source` / `public-read`) comes from **`resolveLiveRepoAnnouncement`** — the same `/api/nostr/profile-repos` path file fetch already uses — plus a live subscribe overlay. Do not treat “files loaded from the owner’s GRASP host” as unpublished. `d` tag match is case-insensitive. Watch uses `a` tag `30617:owner:d` (not the event id). The Star button must stay labeled **Star** (not “Looking up…”) while that id is resolved; lookup effects must not restart when file fetch rewrites the same `repo` object.
- **Remote signer (NIP-46 / Amber):** Star and Watch warm bunker sockets with `ensureRpcHealthy` at click start (same as Push), then sign via `resolveNostrSigner({ waitForRemote: true })`. After hydrate, sockets start CLOSED — without an early warm, Star failed under Code-tab file-fetch with “Could not open any bunker relay”. Keep Amber open/unlocked when starring. The Star button disables while a sign is in flight so double-clicks cannot thrash the bunker dial.
