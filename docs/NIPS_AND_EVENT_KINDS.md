...
- **Owner announce (no gittr hosting)**: Same NIP-82 trio (kinds `32267` / `30063` / `3063`) from Code sidebar **Nostr Apps** (always the **latest** forge Release tag) or Releases → **Announce on Nostr** on a chosen tag — never a tagless app. gittr reads the linked forge’s **Releases** API (GitHub / Codeberg / GitLab) — not a git branch — requires at least one **announceable binary** (APK, AppImage, DMG, linux `tar.gz`, MSI/EXE, IPA, …), hashes it server-side without storing the file, then the owner signs. Default asset `url` is the forge’s public download URL. Optional **pin to public Blossom** (`POST /api/repo/forge-release-blossom-pin`) streams that same file to **`https://blossom.primal.net`**, **`https://blossom.ditto.pub`**, and **`https://haven.danconwaydev.com`** using a signed kind `24242` (`t=upload`, `x=<sha256>`, `expiration`, optional `server`). gittr never writes the bytes to disk and **never** uploads Apps/APKs to **`blossom.gittr.space`** (that origin stays Pages / nsite only). Pin failure does not block announce. **Prefer an `.apk` when present** so Zapstore Android can list it; linux-only tarballs still appear on gittr `/apps`. **Zapstore listing is free** (no fee); if `relay.zapstore.dev` rejects a first publish, commit a `zapstore.yaml` with `repository` + owner `pubkey` for their auto-whitelist ([Zapstore publish docs](https://zapstore.dev/docs/publish)). Extra Zapstore-client trust (APK cert / NIP-C1) is outside this announce path. Do **not** reuse the Pages Blossom proxy (4 MiB JSON limit) for installers.
- **gittr’s own Android listing**: the website/PWA is the product; Zapstore-class stores still need a GitHub Release **APK**. Tag `v*` builds `space.gittr.app` (WebView of gittr.space) via `.github/workflows/build-apk.yml` / `android-app/`. Root `zapstore.yaml` is the whitelist file. Announce that GitHub tag from this repo’s Releases tab like any other forge app.
