...
- **Profile Pages & Apps**: After Repositories, `/{npub}` also shows that person’s **gittr Pages** (gateway status sites) and **NIP-82 Apps** (software catalog), filtered to their pubkey. The author-scoped catalog still fetches `#p` republications (Zapstore often lists the developer in `p`), then **collapses to one card per package id**, preferring the owner’s own kind `32267` over a third-party copy. Dead `cdn.zap.store` icon URLs are dropped at parse (that host no longer resolves). Component: `ui/src/components/profile/ProfilePagesAppsSections.tsx`. Helper: `preferOwnerSoftwareApps`.
