...
- **Owner announce (no gittr hosting)**: Same NIP-82 trio (kinds `32267` / `30063` / `3063`) from Code sidebar **Nostr Apps** (always the **latest** forge Release tag) or Releases → **Announce on Nostr** on a chosen tag — never a tagless app. gittr reads the linked forge’s **Releases** API (GitHub / Codeberg / GitLab) — not a git branch — requires at least one **announceable binary** (APK, AppImage, DMG, linux `tar.gz`, MSI/EXE, IPA, …), hashes it server-side without storing the file, then the owner signs. Default asset `url` is the forge’s public download URL. Optional **pin to public Blossom** (`POST /api/repo/forge-release-blossom-pin`) streams that same file to **`https://blossom.primal.net`**, **`https://blossom.ditto.pub`**, and **`https://haven.danconwaydev.com`** using a signed kind `24242` (`t=upload`, `x=<sha256>`, `expiration`, optional `server`). gittr never writes the bytes to disk. Third-party Apps/APKs are **never** uploaded to **`blossom.gittr.space`** (that host is Pages + the operator gittr APK only). Kind `32267` `image` tags (screenshots) come from the linked forge’s repo-root **`zapstore.yaml` `images:`** (same field `zsp publish` already uses) — repo-relative paths become raw forge HTTPS — plus optional extra HTTPS URLs in the announce panel. `GET /api/repo/zapstore-yaml?sourceUrl=…` fetches only those two filenames (`zapstore.yaml` / `.yml`). Without `images:`, a third-party listing still falls back to duplicating the `icon` as a single `image` tag. **Exception:** the operator’s own gittr Android APK (`space.gittr.app`) may pin there via the same pin API (direct `PUT /upload`, not the 4 MiB Pages proxy). Kind `32267` copies the NIP-34 repo topics (plus zapstore.yaml tags for official gittr); `t=android` is added when an APK is present. Official gittr also sets `icon` to the dark-plate bird PNG on gittr.space and `image` tags to the Zapstore screenshots; other apps copy their Settings logo URL when it is public https, or `icon:` / `images:` from their `zapstore.yaml`. Pin failure does not block announce. **Prefer an `.apk` when present** so Zapstore Android can list it; linux-only tarballs still appear on gittr `/apps`. **Zapstore listing is free** (no fee); if `relay.zapstore.dev` rejects a first publish, commit a `zapstore.yaml` with `repository` + owner `pubkey` for their auto-whitelist ([Zapstore publish docs](https://zapstore.dev/docs/publish)). Extra Zapstore-client trust (APK cert / NIP-C1) is outside this announce path. Do **not** reuse the Pages Blossom proxy (4 MiB JSON limit) for installers.
