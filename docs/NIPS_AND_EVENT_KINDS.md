...
- **API**: `GET /api/repo/forge-releases?sourceUrl=…` (`hash=1` to compute sha256). Soft empty: `no_releases` / `no_apk` → **200** + `ok:false` (most repos). Hard errors: `missing_source`, `unsupported_forge`, forge upstream failures → 4xx/502.
