...
- **Client notes**: Repo zap receive addresses come from the owner’s kind-0 `lud16`/`lnurl` (plus optional repo payment config). Stale `gittr_metadata_cache` used to cause two kinds of “stuck” behavior: it could skip refetch (making it look like there’s no Lightning address), and it could also permanently pin an older cached `lud16/lnurl` even after a newer kind-0 update. gittr now expires the local metadata cache periodically and overwrites cached payment fields when a newer kind-0 is received, lets `/api/zap/create-invoice` resolve `lud16` server-side when the client cache is empty, surfaces real API errors, and prunes the metadata cache on quota errors instead of blaming the recipient.
