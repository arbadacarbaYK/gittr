...
- **Usage in gittr**: The **`/apps`** directory and the repo **Releases** tab both read these kinds. The hub paints the first page of cards then Load more (same window as `/pages`) so chrome navigation stays responsive. Search only hides cards — the live 4000/12000 scrape used to keep running anyway and starve owner-name / header clicks; `/apps` now paints from the server catalog snapshot and pauses catalog updates on leave. **Listed** is unique `pubkey` + app id (`d`). Zapstore’s relay **CLOSED** a kinds-only `limit: 4000` REQ as “filters are too vague” (max kinds-only page is **50**), so gittr used to list only what backup relays had (~509) while two new Zapstore cards were just republishes inside that window. The catalog now **pages** Zapstore with `limit: 50` + `until`, unions with `ui/data/software-catalog-snapshot.json`, slims `raw` so the JSON stays under Next’s 4MB warning, and the hub polls every 8s while that walk is in progress (then every 150s). After a full walk **Listed** can sit around **~3300** because Zapstore’s indexer pubkey republishes a large mobile catalog (~3000 of the cards) — that is still unique publisher+id in **one** merge with every other kind 32267 gittr finds, not Zapstore-count plus a second pile and not 509×5. Republishing the same package id still updates the card without +1. The union snapshot keeps the **oldest** `until` so a later first-wave scrape cannot rewind the cursor. NIP-09 kind 5 is stored on the snapshot (`deletedEventAuthors`) and `/apps` always watches kind 5 even when the server catalog is healthy; only the same author can hide their card. Catalog relays include **`wss://relay.zapstore.dev`**, **`wss://nos.lol`**, **`wss://relay.ngit.dev`**, and `NEXT_PUBLIC_NOSTR_RELAYS` (plus user relays) so Blossom-backed assets resolve like Zapstore / gitworkshop. Download uses the **`url`** on kind `3063` when present (typical for APKs on Blossom or a forge CDN). Gittr does **not** host APKs for this view unless the publisher pointed `url` at your infrastructure. Cards show **kind 0** profile names (and **`p`** attribution tags when present), optional **`license`** (SPDX), and **GitHub stars/forks** when `repository` is a **github.com** URL (server **`POST /api/github/public-repo-stats`**, uses **`GITHUB_PLATFORM_TOKEN`** if set). A **Repo** button is the source you can audit: gittr Code (`/{npub}/{repo}`) when the event has NIP-34 `a` = `30617:<hex>:<repo>` or a gittr.space repository URL, otherwise the NIP-82 `repository` tag (Zapstore’s cloneable git URL — present on almost every listing). Catalog slim keeps that `a` tag (dropping it used to wipe the gittr Code path after a kind-5 refresh). The official gittr listing also gets that path from the operator pubkey + app id when the tag is missing. **Releases** is the forge downloads page when gittr can name one that is not the tree. Catalog slim keeps that `a` tag (it used to drop it, then a kind-5 refresh reparsed every card without **Repo**, including official `space.gittr.app`). The official gittr listing also gets that path from the operator pubkey + app id when the tag is missing.
- **Repo Code About / Releases tab**: The Code-tab **Links** row shows the catalog **app name** (same as `/apps`) and points at `/apps/<id>` (legacy `/apps?q=<id>` still resolves). It is hydrated for every visitor from the last Push and/or the author-scoped catalog — `announcedAppId` is only a local Push hint, not required to *see* the link. Duplicate App rows for the same package id (local vs production origin, catalog vs stored) collapse to one. The **Releases** tab lists **forge** Releases (when a GitHub/Codeberg/GitLab `sourceUrl` exists) **and/or** NIP-82 releases for that owner/repo (matched via `a`=`30617:…`, fuzzy `i` / app id, or stored `announcedAppId`). A forge `sourceUrl` is **not** required to *read* Blossom releases — only for gittr’s **Announce** publish path. **New listing** on that tab is browser notes only (no file attach, no Nostr publish). Nostr-only repos cannot announce an installer from gittr until a hashed file source exists (Blossom upload is not built yet). Announcing does **not** put the APK into the git file tree.
- **Owner announce (no gittr hosting)**: Same NIP-82 trio (kinds `32267` / `30063` / `3063`) from Code sidebar **Nostr Apps** (always the **latest** forge Release tag) or Releases → **Announce on Nostr** on a chosen tag — never a tagless app. Same form as the sidebar (`1.0.0` matches GitHub `v1.0.0`). gittr reads the linked forge’s **Releases** API (GitHub / Codeberg / GitLab) — not a git branch — requires at least one **announceable binary** (APK, AppImage, DMG, linux `tar.gz`, MSI/EXE, IPA, …), hashes it server-side without storing the file, then the owner signs. Default asset `url` is the forge’s public download URL. Optional **pin to public Blossom** (`POST /api/repo/forge-release-blossom-pin`) streams that same file to **`https://blossom.primal.net`**, **`https://blossom.ditto.pub`**, and **`https://haven.danconwaydev.com`** using a signed kind `24242` (`t=upload`, `x=<sha256>`, `expiration`, optional `server`). gittr never writes the bytes to disk. Third-party Apps/APKs are **never** uploaded to **`blossom.gittr.space`** (that host is Pages + the operator gittr APK only). Kind `32267` `image` tags (screenshots) come from the linked forge’s repo-root **`zapstore.yaml` `images:`** (same field `zsp publish` already uses) — repo-relative paths become raw forge HTTPS. Third-party apps may also paste extra HTTPS URLs in the announce panel; official gittr (`space.gittr.app`) does not show that box (shots always come from `https://gittr.space/zapstore/*.png`). `GET /api/repo/zapstore-yaml?sourceUrl=…` fetches only those two filenames (`zapstore.yaml` / `.yml`). Without `images:`, a third-party listing still falls back to duplicating the `icon` as a single `image` tag. **Exception:** the operator’s own gittr Android APK (`space.gittr.app`) may pin there via the same pin API (direct `PUT /upload`, not the 4 MiB Pages proxy). Kind `32267` copies the NIP-34 repo topics (plus zapstore.yaml tags for official gittr); `t=android` is added when an APK is present. Official gittr also sets `icon` to the dark-plate bird PNG on gittr.space and `image` tags to the Zapstore screenshots; other apps copy their Settings logo URL when it is public https, or `icon:` / `images:` from their `zapstore.yaml`. Pin failure does not block announce. **Prefer an `.apk` when present** so Zapstore Android can list it; linux-only tarballs still appear on gittr `/apps`. **Zapstore listing is free** (no fee); if `relay.zapstore.dev` rejects a first publish, commit a `zapstore.yaml` with `repository` + owner `pubkey` for their auto-whitelist ([Zapstore publish docs](https://zapstore.dev/docs/publish)). Extra Zapstore-client trust (APK cert / NIP-C1) is outside this announce path. Do **not** reuse the Pages Blossom proxy (4 MiB JSON limit) for installers.
- **gittr’s own Android listing**: the website/PWA is the product; Zapstore-class stores still need a GitHub Release **APK**. Tag `v*` (official listing **`v1.0.0` and up**, no more `0.x`) builds `space.gittr.app` (WebView of gittr.space) via `.github/workflows/build-apk.yml` / `android-app/`. App id is **`space.gittr.app`** (not `space.gittr.gittr` — that would be a second listing). Root `zapstore.yaml` is the whitelist file (topics: git, nostr, bitcoin, lightning; `images` = `ui/public/zapstore/*.png`). Announce that GitHub tag from this repo’s Releases tab; the form pins the APK to `blossom.gittr.space` by default, sets `icon` to the dark-plate bird PNG (`https://gittr.space/android-chrome-512x512.png`), adds kind `32267` `image` tags for those screenshots, and uses the product about (“Decentralized and discoverable Nostr gits, apps and pages”). That line is also the GitHub repo description; **Refetch from GitHub** copies it. A leftover “Host your Git repositories…” blurb from old GitHub/30617 cache is mapped so refetch cannot restore it. Kind `32267` is replaceable by `d` — keep the same app id on republish. Other apps copy a public Settings logo or the forge `logo.*` file (relative paths like `/logo.svg` are rewritten to a raw GitHub/Codeberg/GitLab URL), and copy `images:` from their own `zapstore.yaml`. Installed APK users tap **Update app** in the user menu; that uses `GET /api/repo/gittr-android-latest` and opens the GitHub APK in the system browser.
- **Delete app announce (not the repo)**: NIP-09 kind `5` referencing the app/release/asset event ids. Separate from deleting a NIP-34 repository. **Your Apps** (`/apps/mine`, user menu) lists the signed-in author’s cards and **Remove listing** deletes one `d` (app id) — e.g. leftover `GITTR` while `space.gittr.app` stays. `/apps` honors these deletions on the **server snapshot** (not only the live fallback scrape) and only when the kind 5 author matches the app. The Code-tab **Links** row (app name) stays so the next Push still publishes it; stray `GITTR` is still dropped so it cannot sit next to `App (space.gittr.app)`. The git repo announcement stays. Blossom/GitHub APK bytes are **not** deleted (same file can still serve the live listing). Zapstore’s relay/client may take its own time (or policy) to drop listings.
- **API**: `GET /api/repo/forge-releases?sourceUrl=…` (`hash=1` to compute sha256; optional `tag=` for a specific Release, else latest). Soft empty: `no_releases` / `no_announceable_asset` (legacy `no_apk`) → **200** + `ok:false` (most repos). Hard errors: `missing_source`, `unsupported_forge`, forge upstream failures → 4xx/502. Full asset lists for the Releases tab use `GET /api/repo/forge-release-list` (no MIME gate). Optional Apps pin: `POST /api/repo/forge-release-blossom-pin` (public Blossom hosts above; `blossom.gittr.space` only for the operator gittr APK). Screenshots: `GET /api/repo/zapstore-yaml?sourceUrl=…` (optional `branch=`). Official Android updater: `GET /api/repo/gittr-android-latest` (GitHub latest `space.gittr.app` APK; used by the user-menu **Update app** item).
- **Implementation**: `ui/src/lib/nostr/nip82-software.ts`, `ui/src/lib/nostr/nip82-repo-releases.ts`, `ui/src/lib/nostr/publish-software-announce.ts`, `ui/src/lib/nostr/nip82-blossom-hosts.ts`, `ui/src/lib/nostr/software-catalog-snapshot.ts`, `ui/src/lib/nostr/software-catalog-zapstore-page.ts`, `ui/src/lib/repo/forge-releases.ts`, `ui/src/lib/repo/gittr-android-app.ts`, `ui/src/lib/repo/gittr-android-shell.ts`, `ui/src/lib/repo/gittr-android-latest.ts`, `ui/src/lib/repo/zapstore-yaml-media.ts`, `ui/src/lib/repo/zapstore-yaml-fetch.ts`, `ui/src/lib/repo/pin-forge-asset-to-ngit-blossom.ts`, `ui/src/lib/nostr/software-catalog-relays.ts`, `ui/src/components/ui/repo-app-announce-panel.tsx`, `ui/src/components/ui/gittr-android-update-menu.tsx`, `ui/src/app/apps/`, `ui/src/app/[entity]/[repo]/releases/`.
- **Interop snippet**: [gittr-helper-tools `nip82-software-announce`](https://gittr.space/npub1n2ph08n4pqz4d3jk6n2p35p2f4ldhc5g5tu7dhftfpueajf4rpxqfjhzmc/gittr-helper-tools?file=snippets/nip82-software-announce/README.md&branch=main)
...
- **Client notes**: Repo zap receive addresses come from the owner’s kind-0 `lud16`/`lnurl` (plus optional repo payment config). Stale `gittr_metadata_cache` used to skip refetch whenever *any* Lightning address was cached (hard refresh kept the old Alby address), pin old `lud16` behind a future/`Date.now()` stamp, and ignore caches that never stored `saved_at`. gittr now always live-fetches kind 0 once per session, expires caches without a timestamp, overwrites payment fields from a newer (or freshly fetched) kind 0 (`kind0-merge.ts`), lets `/api/zap/create-invoice` resolve `lud16` server-side when the client cache is empty, surfaces real API errors, and prunes the metadata cache on quota errors instead of blaming the recipient. `/api/nostr/profiles` TTL is 60s so the HTTP path cannot pin an old lud16 for half an hour.
...
- **Dual-layer with kind 0 (important)**:
  - Kind **0** = profile card (`name`, `display_name`, and camelCase `displayName` some clients write; gittr reads all three before falling back to npub). Text fields such as `nip05`, `picture`, and `lud16` are kept only when they are strings. A number, boolean, array, or object in `nip05` used to crash `/apps` (`nip05.trim is not a function`); those values are dropped when the event is read and again when `gittr_metadata_cache` is loaded. The profile header uses the same `display_name` → `name` order as cards/header. Kind-0 **picture** is the avatar. gittr shows three shapes: a normal `https://` URL (nostr.build, etc.), a Blossom SHA-256 blob (retries `blossom.primal.net` / `blossom.dreamith.to` / `image.nostr.build` when the published host 502s), or an inline `data:image/…` SVG/PNG baked into the event. Kind **10011** is not a second avatar. Repo chrome is separate: NIP-34 `image` / logo file / bridge `/api/og/repo-image`, then the owner’s kind-0 picture, then `/logo.svg`.
...
- **Public profile counts**: `/{npub}` shows **Following** (this pubkey’s kind 3) and **Followers** (authors of kind 3 with `#p` = this pubkey, newest list per author). Visible logged out for legitimacy / WoT context. Kind-3 parsing tolerates non-compliant `p` tags/content that contain `npub1...` instead of hex pubkeys. Many contact lists never appear on NIP-01 websocket relays gittr scrapes (including the author’s NIP-65 list) — Following then falls back to Primal’s HTTP cache `POST ["contact_list", {pubkey}]` via `GET /api/nostr/contact-list`. Hook: `useProfileFollowCounts`. Follower ingest **throttles** recounts (~280ms) and **pauses** on chrome leave so header Apps is not starved while counts still show `…`.
- **Profile Pages & Apps**: After Repositories, `/{npub}` also shows that person’s **gittr Pages** (gateway status sites) and **NIP-82 Apps** (software catalog), filtered to their pubkey. Cards use the same chrome as `/pages` and `/apps` (`GittrPageDirectoryCard` / `SoftwareAppDirectoryCard`) — Globe / Package tile when there is no live icon, not a letter. The author-scoped catalog still fetches `#p` republications (Zapstore often lists the developer in `p`), then **collapses to one card per package id**, preferring the owner’s own kind `32267` over a third-party copy (and borrowing a live icon from the group when the owner’s `icon` was a dead `cdn.zap.store` URL). Dead `cdn.zap.store` icon URLs are dropped at parse (that host no longer resolves); a NIP-82 `image` tag is used when `icon` is missing. Component: `ui/src/components/profile/ProfilePagesAppsSections.tsx`. Helper: `preferOwnerSoftwareApps`.
...
- **Tags**:
  ...
  - `t[]`: Labels (optional)
- **Content**: Markdown description (title lives in `subject`). The change set is git, not JSON in the event — same as ngit / gitworkshop: `clone` + `c` (+ `merge-base` / `refs/nostr/<id>`). gittr’s PR page fetches that range (`GET /api/git/diff`). GitHub `pr-N` rows use `/repos/.../pulls/{n}/files`. Shareable URL is `/pulls/<64-char event id>` on Nostr, or `/pulls/<forge-number>` for GitHub imports (that number is the GitHub PR, not a recency-sorted list index).
- **Interop requirement**: If local cache is missing `r`, derive it from git root commit history (earliest unique commit) before publishing.
