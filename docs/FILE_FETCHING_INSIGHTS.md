...
**Git Server** sidebar picks a preferred clone host by **host priority** (`git.gittr.space`, then ngit / shakespeare / …), not by announcement list order. Clones that appear on the live 30617 `clone` tags always count — including `git.gittr.space` when Push published it. Only timeout-inferred GRASP URLs that are *not* on the event are ignored (so a slow announcement that only lists shakespeare is not overridden by a guessed gittr host). On this deployment, **`git.gittr.space` is the bridge git host**; **`relay.gittr.space` is the Nostr relay** (Pyramid). It stays in `KNOWN_GRASP_DOMAINS` for *reading* other people’s clone tags, but is **not** on `GRASP_SERVERS_FOR_PUSHING` and is not preferred as “Git Server”.
