...
**Crawler path nests:** Empty folders must not render the **root** README with a nested `basePath` (that rewrote relative `src/…` links into deeper `?path=` URLs). `isAbsurdRepoPath` / `sanitizeRepoNavPath` reject deep paths, consecutive duplicate segments, and **any folder name repeated 3+ times** (not only `src`/`docs` — the Aug 2026 outage was `nips/nip52/nips/…/nips`). Same check runs in Next middleware, Code navigation, file-content APIs, and `sanitizeRepoTreePath`. nginx on gittr.space 404s the known query patterns before they reach Node. Use **404** not **429** so crawlers drop the URL.
