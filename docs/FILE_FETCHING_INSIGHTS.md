...
**Profile page avatars** use the same kind-0 `picture` (not the repo logo). `useBlossomMediaSrc` / `blossomMediaFallbackUrls` must pass through `data:image/…` — treating only `http(s)` as valid used to show `/logo.svg` on `/{npub}` while Explore still showed the inline SVG. Helper: `isDisplayableProfilePicture`. Social `og:image` in `[entity]/layout.tsx` still prefers `https` only (inline data URIs are too large for meta tags).
...
**Pages:** owners go live with **Push Manifest** (Blossom + kind 35128, same signer as Push — including Amber). Manifest bytes come from GitHub/the git bridge first; leftover `gittr_files` / `gittr_overrides` in this browser are only a fallback so Refetch + Push Manifest cannot re-publish an old `index.html` while Code-tab preview already shows GitHub. The **gittr** monorepo Page is the docs hub (`index.html`, `docs-site/`, `docs/` media) — `ui/` and other forge trees are skipped so Push Manifest is not ~800 Next.js files. Helper-tools stays a small site next to `index.html`. A README pagelink and custom site name are optional. **Open a tracking issue** is for non-owners only; it is not a pull request and does not publish the site. A custom Pages name is stored in the browser (not on kind 30617); hydrate must not wipe it. NIP-5A names are 1–13 characters because the host is one DNS label (`50-char pubkey` + name ≤ 63). `conference-loop` → `conference-lo`. Names like `gittr` / `gittr-…` are reserved for everyone except the platform npub `npub1n2ph08n4pqz4d3jk6n2p35p2f4ldhc5g5tu7dhftfpueajf4rpxqfjhzmc`. Code-tab HTML preview loads relative CSS via `/api/gittr-pages/preview-asset` (a GitHub `<base>` tag is blocked by CSP). Profile Pages cards match named `{pubkeyB36}{dTag}` hosts, not only `npub1…` root sites. The profile reads `GET /api/gittr-pages/status-sites?author=…` from the live gateway dump — do not use Next’s Data Cache for `manifests.json` (it cannot store >2MB, so the directory froze on an old snapshot).

The **nsite-gateway** (`pages.gittr.space`) is not a third-party cache in front of “the network.” It only lists sites that are in **its** in-memory event store. That store used to hydrate from public relays (Damus, nos.lol, …) every **10 minutes**, and a bulk `since=newest-of-any-site+1` skipped a republish that was older than some other author’s latest row. Links and the profile **do not invent** a Pages URL — they wait for `/status/manifests.json`. So a Push Manifest that landed on `wss://relay.gittr.space` could serve HTML if you already knew the host (per-request fetch via outboxes) while About → Links and the profile stayed empty, and an existing host like `gittr-docu` kept the **old** blob because the store already had that replaceable. Production `NOSTR_RELAYS` must include `wss://relay.gittr.space`, live subscribe (`RELAY_SYNC_INTERVAL=live`), and Push Manifest calls `GET /api/gittr-pages/ingest` so the gateway loads that pubkey+d immediately. Site HTML uses `Cache-Control: max-age=0, must-revalidate` (ETag = blob sha256) so clicking the link is not a one-hour browser cache of the previous `index.html`. **Push Manifest always published to `relay.gittr.space`.** The miss was the gateway not *reading* that relay. After ingest, the success popup uses the same ✅ / ⚠️ / ❌ language as repo Push; if the directory has not listed the host yet it says so (seconds, not 10 minutes). Hub maps point sibling boxes at `*.pages.gittr.space` and stay in the same tab; gittr.space / GitHub / Blossom / Help open in a new tab. `/pages` search filters as you type (including kind-0 author names and the Pages `d` tag), not only after a submit.
