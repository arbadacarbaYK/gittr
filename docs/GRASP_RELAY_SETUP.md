...
- **Issue close / status (`1630`–`1633`):** GRASP write checks must accept a root **kind 1621 issue**, not only patches (`1617`) or PRs (`1618`). An older check rejected `1632` closes with `issue not found: must reference an existing issue or pull request (kind 1617 or 1618)`, so gittr Issues stayed Open on this relay even after a valid close on ngit/shakespeare. Fix lives in the pyramid fork `grasp/validation.go`.
...
A home Umbrel / Tailscale / `*.local` address can live in your NIP-65 or kind **10317** list for other clients. **gittr.space will not open those sockets** (Chrome local-network prompt). The Code tab **does** still dial the owner’s other public `relays` tags — that is required for repos that only exist on a custom public relay. Public clone hosts (ngit, shakespeare, nostrhub) are still used. Do not put `umbrel.local` on a public kind **30617** `relays` tag — it resolves to *each visitor’s* Umbrel, not yours.
