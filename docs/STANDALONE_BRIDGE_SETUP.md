...
| `gitRepoOwners` | optional | If empty, the bridge watches **all** authors’ NIP-34 events. Bare repos materialize only when `clone[]` includes this GRASP host (`git.gittr.space`). |
