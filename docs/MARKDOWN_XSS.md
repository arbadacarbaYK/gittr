...
`ReadmeMarkdownImage`. Status-badge hosts (`img.shields.io`, badgen, …) render
inline in one paragraph like GitHub; screenshots keep the scrollable block wrap.
A same-folder name like `![…](file-fetch.gif)` is joined
to the **markdown file’s directory**. Forge `sourceUrl` hotlinks are best-effort; SVG and failed
