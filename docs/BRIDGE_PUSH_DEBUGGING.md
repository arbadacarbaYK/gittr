...
Clone URL is the announcement’s `clone[]` plus forge `source`. **has files** means that listed host currently has git refs (`git ls-remote --heads` via `/api/git/clone-heads`), not “GitHub won this visit’s file tree.” After a gittr Push, `git.gittr.space` must badge when the bridge has commits even if the Code tab painted from GitHub. One row per host (`.git` / no-`.git` collapse). If the list flashes **all event hosts → GitHub only → a subset**, a later `setRepoData` dropped `announcementClone` and a fetch run with an empty URL array remapped `present` → `empty` and inferred 4 GRASP. Keep `announcementClone` across GitHub-first / import / localStorage rebuilds; never infer when the 30617 already had clone tags.
