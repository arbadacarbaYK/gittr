...
**Blossom host (nostr.build vs Pages):** **nostr.build** is aimed at **media** uploads; its Blossom endpoint often returns **415** for static site assets such as **`.js`** (“file type not allowed”). That is **not** a gittr MIME bug — the upstream **refuses the file class**. For **Publish Pages manifest**, point **`NEXT_PUBLIC_BLOSSOM_URL`** at **`https://blossom.band`** (code default) or another **NIP-96** Blossom that allows your MIME set; **self-hosting** (e.g. **[hzrd149/blossom](https://github.com/hzrd149/blossom)**) is optional if you want full control. On the server, change **`/opt/ngit/ui/.env`** (or wherever the service loads env), then **`yarn build`** and **`systemctl restart gittr-frontend`**.
