...
- **App announce (NIP-82 / Zapstore)**: owners publish from the repo Code sidebar (**Nostr Apps** = latest forge Release tag) or Releases tab (**Announce on Nostr** on a chosen tag). Same events either way; never a tagless app. Requires a linked GitHub/Codeberg/GitLab/Forgejo `sourceUrl` with a forge **Release** that includes an announceable binary (APK preferred for Zapstore Android; AppImage/DMG/`tar.gz`/MSI/EXE/IPA also work). Optional sibling binaries on the same tag with verified sha256 are extra NIP-82 assets. gittr does **not** store binaries. Default kind `3063` `url` is the forge download. Optional checkbox **pin to public Blossom** streams the file through gittr (no disk) to `blossom.primal.net`, `blossom.ditto.pub`, and `haven.danconwaydev.com`, and rewrites `url` if that succeeds; pin failure still publishes the forge URL. **Third-party apps cannot pin to `blossom.gittr.space`.** The operator gittr repo (`space.gittr.app`) can — that pin is on by default, and the announce includes the bird logo (`icon` on kind `32267`). Announce copies the repo’s NIP-34 topics onto kind `32267`. `POST /api/repo/forge-release-blossom-pin` is the pass-through (signed kind `24242`). `GET /api/repo/forge-releases` is announce-gated (optional `tag=`; omit = latest). `GET /api/repo/forge-release-list` is the Releases tab asset list. The Releases tab also **reads** NIP-82 kinds `30063`/`3063` without a forge `sourceUrl`. **New listing** on Releases is browser notes only (no file attach). Nostr-only repos cannot announce an installer from gittr until hashed-file upload exists. Optional `GITHUB_PLATFORM_TOKEN` improves GitHub rate limits. Operator-facing help: `/help#releases` and `/help#publish-pages-apps`. Integrator notes: [gittr-helper-tools `nip82-software-announce`](https://gittr.space/npub1n2ph08n4pqz4d3jk6n2p35p2f4ldhc5g5tu7dhftfpueajf4rpxqfjhzmc/gittr-helper-tools?file=snippets/nip82-software-announce/README.md&branch=main). See NIP-82 section in [NIPS_AND_EVENT_KINDS.md](NIPS_AND_EVENT_KINDS.md).
