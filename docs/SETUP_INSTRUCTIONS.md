...
- **`/explore`** uses the same NIP-34 discovery set immediately (`exploreRepoRelaysForClient`), not only gittr’s own relay plus whatever `relays` tags happen to arrive first. That used to make the page look like “only bridge repos” for a long time: the SEO seed + Pyramid answered fast, while gitworkshop.dev / `git.gittr.space` were wrongly dialed as WebSockets and starved ngit / Shakespeare / NostrHub. Extra discovered relays are subscribed **once** per host. Verbose Explore logs: `localStorage.gittr_explore_debug=1`.
