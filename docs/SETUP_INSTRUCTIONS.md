...
**Notifications (kind 30078):** Settings → Notifications → Save publishes kind **30078** `d=gittr/notifications` with all channel + event toggles (multi-browser). Telegram User ID registers via `POST /api/notifications/consent` into **server-only** `data/notifications-consent.json` (not on relays). Recipient DMs go through `POST /api/notifications/deliver` (looks up the **recipient’s** consent — not the actor’s localStorage). Legacy `d=gittr/security-cve` is still hydrated. **CVE bot (live):** daily `gittr-cve-bot.timer` with **`CVE_BOT_ENABLED=1`** in `/etc/default/gittr-cve-bot` scans opted-in owners and opens one security issue + DM for each new eligible finding. Opted-in = `events.security_cve` on 30078. **Public issue links** use `https://gittr.space/npub1…/reponame/issues/{id}` (`nip19.npubEncode` via `buildCvePublicIssueUrl`) — never hex pubkeys in the path; DM/issue copy uses the human repo name only. **Freshness:** bot only alerts when gittr bridge tip **equals** Nostr kind **30618** HEAD tip; no tip or tip mismatch → skip. Users should **sync from source (if GitHub/forge moved) + Push** so the announcement matches the mirror — browser-only refetch without Push does not fix mismatch. **Dedup:** one alert per `owner|repo|advisory|package` forever after successful send (Refetch/Push does not re-spam). Help: `/help#security-alerts`.
