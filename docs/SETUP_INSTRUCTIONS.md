...
**Notifications (kind 30078):** Settings → Notifications → Save publishes kind **30078** `d=gittr/notifications` with all channel + event toggles (multi-browser). Telegram User ID registers via `POST /api/notifications/consent` into `data/notifications-consent.json` (not on relays). Recipient DMs go through `POST /api/notifications/deliver` (looks up the **recipient’s** consent — not the actor’s localStorage). Legacy `d=gittr/security-cve` is still hydrated. CVE bot: `npx tsx scripts/cve-bot.mts` (live only with `CVE_BOT_ENABLED=1`); opted-in = `events.security_cve` on 30078; delivers via `/api/notifications/deliver`.

**CVE bot systemd (Hetzner):** units in `infra/systemd/gittr-cve-bot.{service,timer}` → `/etc/systemd/system/`. Config: `/etc/default/gittr-cve-bot` with `CVE_BOT_ENABLED=1` for live publish (omit or `0` = dry-run). Timer runs daily; `systemctl start gittr-cve-bot.service` for a one-shot. Logs: `journalctl -u gittr-cve-bot -n 100`. Dedup: `/opt/ngit/data/cve-bot-dedup.json`.
