...
- Owner **names** (vs short npub) come from kind **0**. `POST /api/nostr/profiles` scrapes profile relays, then falls back to the Primal cache HTTP index when websocket misses (same reason njump shows a name gittr used to show as `npub1…`). **Reading** kind 0 is a public REQ/HTTP fetch — Amber / NIP-46 is only needed to **publish** Profile Settings, not to show your name and picture. A hydrate-only remote-signer session must still load names. In-memory TTL is **60s** (not 30 minutes) so a new `lud16` is not pinned. Do **not** cache empty misses. Name fields: `display_name`, camelCase `displayName`, then `name`. Lightning on the profile header is that same kind 0 — the client must refetch even when localStorage already has an address. Welcome / header show that kind-0 name, and fall back to a short `npub1…` when kind 0 has no name. Profile Settings Username is still kind-0 `name` only — do not prefill the truncated npub into the editable handle (credentials already show the npub).
