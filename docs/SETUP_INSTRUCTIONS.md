...
- `NEXT_PUBLIC_NOSTR_RELAYS` should match bridge `relays` in `git-nostr-bridge.json`. Explore dials gittr’s NIP-34 set (`relay.gittr.space`, ngit, shakespeare, …) plus the visitor’s Settings → Relays (NIP-65) list. It does **not** auto-dial announce `relays` tags, dead GRASP (`git-01.uid.ovh`, `ngit-relay.nostrver.se`), or `/grasp` clone paths as WebSockets. **Public origin (gittr.space):** never open WebSockets or clone fetches to localhost, `.local`, RFC1918, Tailscale `*.ts.net`, or CGNAT `100.64/10` — even if they appear in NIP-65, kind 10317, or a NIP-34 `relays`/`clone` tag. Public owner GRASP (`git.shakespeare.diy`, `relay.ngit.dev`, `git.nostrhub.io`, …) is never stripped. Some clients (e.g. shakespeare.diy) copy a person’s full NIP-65 — including `umbrel.local` — into kind **30617** `relays`; gittr will not dial those from gittr.space, will not `git clone` them on Hetzner, and will not re-publish them. Self-hosted gittr on localhost may still dial *your* home relays from NIP-65/env. CSP `connect-src` uses `wss:` / `ws:` so custom-port relays can connect.
