...
**Nginx / 502 on Blossom proxy:** If users see **502** on **`/api/gittr-pages/blossom-proxy-upload`** while publishing a manifest, nginx may be timing out the Next.js upstream (default **60s** `proxy_read_timeout`) or the public Blossom tier may return **502/503** briefly. The proxy route sets **`maxDuration` 120s** and explicit **`Content-Length`** on the upstream PUT; the client retries **502/503/504/429** a few times. On the server, set **`proxy_read_timeout`** / **`proxy_send_timeout`** to **≥180s** on locations that **`proxy_pass`** to the Next.js port (see **`nginx.gittr.conf.example`**). Reload nginx after editing.
