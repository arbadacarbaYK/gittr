...
- **GitHub / Forgejo upstream tabs**: Issues/PRs/Releases soft-refresh from GitHub when `sourceUrl` is github.com, and from Forgejo/Gitea `/api/v1` (including Codeberg and self-hosted) for those hosts. **GitLab issues/MRs are not live-synced** (files and Releases still work). Commits and GitHub Projects V2 stay GitHub-only. The ToDo tab **Quick Add** list uses that same open-issue cache (closed GitHub tickets are not offered). Releases persist in `gittr_releases__*` so the tab works without a prior Code visit. Optional `GITHUB_PLATFORM_TOKEN` improves GitHub rate limits.
- **Discussions (NIP-23 kind 30023)**: the thread is on relays. This browser keeps a cache in `gittr_discussions__*` (legacy `gittr_discussions_<entity>_<repo>` is still read). A `QuotaExceededError` on save is **origin localStorage** (~5–10MB), not a signing or relay quota. After a successful publish, gittr opens the event even if the cache write fails, and owner Delete is NIP-09 kind 5. Dependencies scan skips `*.test.*` / `node_modules` (those console lines were not filling storage).
