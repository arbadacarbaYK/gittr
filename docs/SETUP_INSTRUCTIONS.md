...
**Blossom MIME (415 / 400):** Public Blossom (e.g. nostr.build) **sniffs bytes** and compares them to the **`Content-Type`** on **`PUT /upload`**. Sending **`application/octet-stream`** can cause **415**; some hosts **allowlist** types so **`text/javascript`** is used for real `.js` blobs (we saw **415** when using **`application/javascript`**). Sending **`text/javascript`** while the body is a **valid JSON document** (e.g. **`JSON.stringify(source)`** stored under `.js`) causes **400** (“expected **application/json**”). Gittr uses **`blossom-upload-mime.ts`**: **strict-JSON sniff** → **`application/json`**, optional **unwrap** of JSON-stringified script bytes for `.js`/`.mjs`/`.cjs` before hashing/upload, path fallback **`text/javascript`** for scripts, and **`blossom-proxy-upload`** **reconciles** client `contentType` vs decoded bytes. If a type still fails, try another **`NEXT_PUBLIC_BLOSSOM_URL`** or confirm stored bytes are the real asset (not an API error JSON body).
