...
- **PR file changes:** kind **1618** and GitHub-imported `pr-N` rows often have no `changedFiles` in the browser. The PR page fetches them: GitHub via `/api/github/proxy` `/pulls/{n}/files`; Nostr via **`GET /api/git/diff?sourceUrl=&head=&base=&ref=`** (temp clone of the event’s `clone` URL, `c` vs `merge-base`). Codeberg/Gitea can pass **`pullNumber`** so the API looks up the PR SHAs. Same `gitFetch` rate bucket as `/api/git/repo-files`.
