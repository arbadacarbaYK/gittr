...
- **GitHub upstream tabs**: Issues/PRs/Commits/Releases soft-refresh from GitHub when `sourceUrl` is known (not only at Push/import). Repo **ToDo** tab also imports **GitHub Projects V2** (read-only) via GraphQL (`POST /api/github/graphql`); optional `GITHUB_PLATFORM_TOKEN` improves rate limits / visibility.
