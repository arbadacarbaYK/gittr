...
**Publish Pages manifest (Blossom + kind 35128):** Owners get **Publish Pages manifest** in the **gittr Pages** sidebar. The client signs **NIP-07** kind **24242** tokens per file, then `**POST /api/gittr-pages/blossom-proxy-upload`** forwards `**PUT /upload**` to the **Pages Blossom origin**: **`NEXT_PUBLIC_GITTR_PAGES_BLOSSOM_URL`** if set, else **`NEXT_PUBLIC_BLOSSOM_URL`**, else **`https://blossom.band`**. There is **no** automatic host switching — whatever you configure is what runs. Media-only Blossoms (e.g. some **nostr.build** NIP-96 endpoints) return **415** for **`.js`** / HTML; point the env at a static-friendly Blossom (e.g. **`https://blossom.band`**) or your own server. Use **`NEXT_PUBLIC_GITTR_PAGES_BLOSSOM_URL`** when **general** Blossom traffic should stay on one host but **Pages** should use another.
