...
- Owner **names** (vs short npub) come from kind **0**. `POST /api/nostr/profiles` scrapes profile relays, then falls back to the Primal cache HTTP index when websocket misses (same reason njump shows a name gittr used to show as `npub1…`). In-memory TTL is **60s** (not 30 minutes) so a new `lud16` is not pinned. Do **not** cache empty misses. Name fields: `display_name`, camelCase `displayName`, then `name`. Lightning on the profile header is that same kind 0 — the client must refetch even when localStorage already has an address.
...
**Pyramid / relay.gittr.space:** add **`30078`** to production `open_kinds_spec` (see pyramid `FORGE.md` / Settings UI), then `systemctl restart pyramid` — otherwise non-members cannot publish prefs to the forge relay. Help: `/help#security-alerts`. The HTTPS dashboard is Pyramid’s Tailwind UI; if `https://relay.gittr.space/` looks like unstyled 1990s HTML, `/static/styles.css` was not embedded — rebuild with Tailwind (`easy.sh` / `just build`), see [GRASP_RELAY_SETUP.md](GRASP_RELAY_SETUP.md).
