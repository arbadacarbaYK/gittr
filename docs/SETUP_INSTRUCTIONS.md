...
- Code tab file list: **`main` and `master` are the same default tip.** A successful clone listing (`51 files from pyramid…`) must paint even when git resolved `master` and the dropdown still says `main`. Empty local never blocks that. Do not clear `isInProgress` / retry-storm `/api/git/repo-files` just because clone URLs exist and the tree is still empty — wait for the in-flight race (retry budget 2). Console `Keeping local tree instead of remote (N)` with `0 files` on screen was that apply-gate bug (see [FILE_FETCHING_INSIGHTS.md](FILE_FETCHING_INSIGHTS.md)).
...
| Empty Code tab but console shows files fetched (`Keeping local tree`) | `main` vs `master` apply-gate — listing must paint; see FILE_FETCHING_INSIGHTS “Which tree” |
