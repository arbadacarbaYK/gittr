...
- Owner **names** (vs short npub) come from kind **0**. `POST /api/nostr/profiles` scrapes profile relays, then falls back to the Primal cache HTTP index when websocket misses (same reason njump shows a name gittr used to show as `npub1…`). Do **not** cache empty misses. Name fields: `display_name`, camelCase `displayName`, then `name`.
