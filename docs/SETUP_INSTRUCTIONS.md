...
**Blossom 415 (unsupported file type):** Some public Blossom endpoints (e.g. nostr.build) **reject** blobs when **`Content-Type` is wrong** — sending everything as `application/octet-stream` causes **415** for `.js`, `.css`, etc. The app sends a **type derived from the file path** on each upload (see `guessManifestFileContentType` in `publish-named-site-manifest.ts` and `blossom-proxy-upload/route.ts`). If 415 persists for a given extension, that host’s allowlist may not include it; point **`NEXT_PUBLIC_BLOSSOM_URL`** at a Blossom that accepts static site assets, or host blobs elsewhere.
