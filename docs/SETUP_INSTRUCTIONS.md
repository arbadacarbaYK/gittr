...
- **`/explore`** first-subscribes the NIP-34 / GRASP set (`exploreImmediateDiscoveryRelays`: gittr, ngit, Shakespeare, NostrHub, gitnostr, …) and only then Damus/wine/etc. Seed/SEO cards are the long-indexed snapshot and must not look like “done.” Extra discovered relays are subscribed **once** per host. Verbose Explore logs: `localStorage.gittr_explore_debug=1`.
