...
**If header / repo tabs feel dead until the file list + README finish:** that is **main-thread Code-tab hydrate** (tree dates + ReactMarkdown), not a CSS overlay. Leaving Code hard-navigates for **all** chrome (logo, top nav, user menu, owner link, Issues/Settings tabs). Large READMEs show a plain preview + **Show formatted README** instead of auto-parsing (markdown parse was freezing clicks). Small READMEs still idle-gate and cancel if you click chrome first. Soft `router.push` elsewhere keeps a ~400 ms hard fallback.
