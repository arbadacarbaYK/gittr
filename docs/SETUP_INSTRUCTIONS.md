...
- `NEXT_PUBLIC_NOSTR_RELAYS` should match bridge `relays` in `git-nostr-bridge.json`. Explore dials gittr’s NIP-34 set (`relay.gittr.space`, ngit, shakespeare, …) plus the visitor’s Settings → Relays (NIP-65) list. It does **not** auto-dial announce `relays` tags, dead GRASP (`git-01.uid.ovh`, `ngit-relay.nostrver.se`), or `/grasp` clone paths as WebSockets. CSP `connect-src` uses `wss:` / `ws:` so custom-port relays can connect.
