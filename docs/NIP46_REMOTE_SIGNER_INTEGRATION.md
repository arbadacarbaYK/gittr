...
10c. **First sign after hydrate must warm bunker transport** - `ensureRpcHealthy` calls `prepareTransportForSession` → `ensureBunkerSocketsOpen` (expand thin relay lists with Amber defaults, **claim bunker hosts** so the app relaypool cannot re-`addRelay` them, free colliding main-pool sockets **before** the first dial, open `directPool` sockets in **parallel** with a **10s per-relay budget**, return on first OPEN, start 24133 subscription) before `reestablishConnection`. Page load also schedules a **background warm** (idle + ~45s keepalive) that shares the same dial mutex with Push so they cannot `resetDirectPool` under each other. On total failure, reset the pool and force Amber-friendly defaults; log per-relay status. Otherwise hydrate-only leaves zero OPEN bunker sockets and Save/Push fails with “Could not open any bunker relay”.
11. **NIP-46 uses `directPool` only** - never `addRelay` / subscribe / publish bunker hosts through the main NostrContext pool on hydrate/sign/sendRequest (duplicate sockets starve Amber). Relays Settings shows kind **10002** for display/edit and must not dump that full list into the app pool either. Adapter `getRelays()` is transport metadata, not user prefs. Damus is a last-resort bunker fallback (Cloudflare often fails it in browsers).
