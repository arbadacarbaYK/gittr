...
10c. **First sign after hydrate must warm bunker transport** - `ensureRpcHealthy` calls `prepareTransportForSession` (expand thin relay lists with Amber defaults, open `directPool` sockets with a **15s per-relay budget**, start 24133 subscription) before `reestablishConnection`. Do **not** kill CONNECTING sockets with a short Promise.race (that was closing Amber's path before WebSocket OPEN). On total failure, reset the pool and retry once; log per-relay status. Otherwise hydrate-only leaves zero OPEN bunker sockets and Save/Push fails with “Could not open any bunker relay”.
