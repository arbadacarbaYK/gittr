...
10c. **First sign after hydrate must warm bunker transport** - `ensureRpcHealthy` calls `prepareTransportForSession` → `ensureBunkerSocketsOpen` (expand thin relay lists with Amber defaults **ephemerally for dialing only** — do **not** persist expansion into `session.uriRelays` / overwrite Amber’s URI list, **claim bunker hosts** so the app relaypool cannot re-`addRelay` / `subscribe` them, free colliding main-pool sockets **by the pool’s real keys** then wait until those entries are gone, open `directPool` sockets in **parallel** with a **15s per-relay budget**, **proceed on the first OPEN** so Push is not blocked waiting for all URI hosts, start 24133 subscription). Keepalive continues filling the rest. **Never `resetDirectPool` while any bunker socket is CONNECTING (status 0)** — that was killing in-flight handshakes and logging `No OPEN bunker sockets` / `Could not open any bunker relay`. When a cached user pubkey exists and at least one bunker socket is OPEN, skip a blocking reconnect `connect` — Amber often ignores silent reconnect acks while still answering `sign_event`. Mark `rpcHealthy` only after a successful RPC. Page-load hydrate **dials immediately** in the same parent effect that constructs the manager (before child file-fetch subscribes). Background warm failures stay **debug-level** (Push still hard-fails with a clear error).
