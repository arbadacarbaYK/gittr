...
10c. **First sign after hydrate must warm bunker transport** - `ensureRpcHealthy` calls `prepareTransportForSession` (expand thin relay lists with Amber defaults, open `directPool` sockets in **parallel** with an **8s per-relay budget**, return on first OPEN, start 24133 subscription) before `reestablishConnection`. Page load also schedules a **background warm** (idle + ~45s keepalive) so Push is not the first cold dial. On total failure, free main-pool sockets that collide with bunker hosts, reset the pool, and retry once; log per-relay status. Otherwise hydrate-only leaves zero OPEN bunker sockets and Save/Push fails with “Could not open any bunker relay”.
