...
Explore / My Repositories / Profile / Pages / Apps grids use shared **Load more** pagination (48 cards at a time). `/explore` first paints the SEO snapshot + pushed-repos file (fast, even if that snapshot is a couple of weeks old), then fills in the rest from NIP-34 discovery relays (ngit, Shakespeare, NostrHub, gitnostr, gittr) — not only repos that live on this deployment’s git bridge. Leaving Explore and coming back, or searching from the header, keeps that live list in the tab (a full refresh still starts from sessionStorage + the seed, then localStorage).
