...
Explore / My Repositories / Profile / Pages / Apps grids use shared **Load more** pagination (48 cards at a time). `/pages` cards use the same three-across layout as `/apps` (author picture, nip05, TrustBadge, hostname, snapshots) from gateway + kind-0 metadata — not a new API. `/apps` still loads the full catalog snapshot from the server, then paints 48 cards — search only hides cards, it does not shrink the catalog. After that snapshot, the hub does not keep a live 4000/12000 NIP-82 scrape running (that used to ignore owner-name and header clicks), but it still watches NIP-09 kind 5 so **Remove listing** hides cards. Vanity `/DisplayName/repo` URLs (the Share copy under the QR) 307 to `/{npub}/repo` in the browser so files/Push work; X/Telegram stay on the name URL so the preview still has the About text. Owner names on `/apps` and `/pages` open the publisher profile in a new tab. Each card has a share icon beside the title (Apps shares `/apps/{id}`; Pages shares the live site). `/explore` first paints the SEO snapshot + pushed-repos file (fast, even if that snapshot is a couple of weeks old), then fills in the rest from NIP-34 discovery relays (ngit, Shakespeare, NostrHub, gitnostr, gittr) — not only repos that live on this deployment’s git bridge. Leaving Explore and coming back, or searching from the header, keeps that live list in the tab (a full refresh still starts from sessionStorage + the seed, then localStorage).
