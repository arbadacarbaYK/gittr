...
export default function SettingsLayout({
  ...
  useEffect(() => {
    ...
    if (actualName && actualName !== "Anonymous Nostrich") {
      ...
      actualDisplayName &&
      actualDisplayName !== "Anonymous Nostrich"
    ) {
        ...
        <div className="flex justify-between">
          <div className="space-x-4 items-center flex">
            ...
            <header suppressHydrationWarning>
              ...
              {mounted && actualName && (
                <h3 className="text-zinc-500 text-xs">@{actualName}</h3>
              )}
              {mounted && metadata.nip05 && (
                <h3 className="text-zinc-500 text-xs">{metadata.nip05}</h3>
              )}
            </header>
          </div>
        </div>
