...
export default function BountiesSettingsPage() {
  ...
  useEffect(() => {
    try {
      ...
      stored.sort(
        (a: BountyEntry, b: BountyEntry) => b.createdAt - a.createdAt
      );
      setBounties(stored);
    } catch (error) {
    ...
    switch (bounty.status) {
      ...
      case "claimed":
        return bounty.claimedBy
          ? `Claimed by ${bounty.claimedBy.slice(0, 8)}... on ${formatDate24h(
              bounty.claimedAt || 0
            )}`
    ...
    <div className="p-6">
      ...
      <div className="mt-6">
        <p className="text-sm text-gray-400 mb-4">
          Track all bounties you've offered on issues. You'll be notified when
          someone claims or fulfills them.
        </p>
          ...
          <div className="space-y-4">
            {bounties.map((bounty) => {
              ...
              return (
                <div
                  key={bounty.issueId}
                  className="p-4 border border-gray-700 rounded bg-gray-900"
                >
                  ...
                  {bounty.claimedBy && bounty.status === "claimed" && (
                    <div className="mt-3 p-2 bg-yellow-900/20 border border-yellow-700 rounded text-sm">
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        When they merge a PR that fixes this issue, the bounty
