...
import { type Activity, getActivities } from "@/lib/activity-tracking";
...
export default function InsightsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (foundRepo) {
        ...
        console.warn("Insights: Repo not found", {
          entity,
          repoName,
  ...
  const stats = useMemo<RepoStats | null>(() => {
    ...
    try {
      ...
      const issues = parseJsonArray<StoredIssue>(
        localStorage.getItem(issueKey)
      );
      const openIssues = issues.filter(
        (issue) => getStatus(issue.status, "open") === "open"
      ).length;
      const closedIssues = issues.filter(
        (issue) => getStatus(issue.status, "open") === "closed"
      ).length;

      // Load PRs count
      const prKey = `gittr_prs__${entity}__${repoName}`;
      const prs = parseJsonArray<StoredPullRequest>(
        localStorage.getItem(prKey)
      );
      const openPRs = prs.filter(
        (pr) => getStatus(pr.status, "open") === "open"
      ).length;
      const mergedPRs = prs.filter(
        (pr) => getStatus(pr.status, "open") === "merged"
      ).length;
      const closedPRs = prs.filter(
        (pr) => getStatus(pr.status, "open") === "closed"
      ).length;

      // Load commits from activity tracking
      const activities = getActivities();
      const repoActivities = activities.filter(
        (activity: Activity) => activity.repo === repoId
      );
      const commitCount = repoActivities.filter(
        (activity) => activity.type === "commit_created"
      ).length;

      // Load discussions count
      const discussionKey = `gittr_discussions_${entity}_${repoName}`;
      const discussions = parseJsonArray<unknown>(
        localStorage.getItem(discussionKey)
      );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="border border-[#383B42] rounded p-4 bg-[#171B21]">
          ...
          <p className="text-3xl font-bold">
            {repoData?.files
              ? repoData.files.filter((file) => file.type === "file").length
              : 0}
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <div className="border border-[#383B42] rounded p-4 bg-[#171B21]">
          ...
          <p className="text-sm text-gray-400">
            {stats?.issues.open || 0} open, {stats?.issues.closed || 0} closed
          </p>
        </div>
        ...
        <div className="border border-[#383B42] rounded p-4 bg-[#171B21]">
          ...
          <p className="text-sm text-gray-400">
            {stats?.prs.open || 0} open, {stats?.prs.merged || 0} merged
          </p>
        </div>
