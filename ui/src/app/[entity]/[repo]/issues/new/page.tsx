...
import { Check, ChevronDown, Edit, Settings, X, Coins } from "lucide-react";
...
export default function RepoIssueNewPage() {
  ...
  const [availableMilestones, setAvailableMilestones] = useState<Array<{id: string; name: string}>>([]);
  ...
  const [bountyWithdrawUrl, setBountyWithdrawUrl] = useState<string | null>(null);
  ...
  const [bountyPaymentHash, setBountyPaymentHash] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    try {
      ...
      const repoData = repos.find((r: any) => r.entity === entity && r.repo === repo);
      if (repoData && repoData.topics) {
        // Use repo topics as available labels
        const labels = repoData.topics.filter((t: any) => typeof t === "string" ? t : t?.name).filter(Boolean);
      ...
      try {
        ...
        const projects = JSON.parse(localStorage.getItem(projectsKey) || "[]") as any[];
      ...
      try {
        ...
        const milestones = JSON.parse(localStorage.getItem(milestonesKey) || "[]") as Array<{id: string; name: string; dueDate?: number; description?: string}>;
        setAvailableMilestones(milestones.map((m: any) => ({ id: m.id, name: m.name })));
      } catch {
      ...
      if (repoData && repoData.contributors && Array.isArray(repoData.contributors)) {
        ...
        const validContributors = repoData.contributors
          .filter((c: any) => c.pubkey && typeof c.pubkey === "string" && c.pubkey.length === 64 && /^[0-9a-f]{64}$/i.test(c.pubkey))
          .map((c: any) => ({
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (!hasNip07 && !privateKey) {
        if (!isLoggedIn) {
          ...
          setErrorMsg("To create issues, you need either: 1) A NIP-07 browser extension (e.g., Alby) installed and enabled, or 2) A private key configured in Settings.");
        }
        setTimeout(() => {
          setErrorMsg("");
        }, 8000);
      ...
      try {
        ...
        try {
          ...
          window.dispatchEvent(new CustomEvent("gittr:issue-created", { detail: newIssue }));
          ...
          try {
            ...
            if (ownerPubkey) {
              const notification = formatNotificationMessage("issue_opened", {
                ...
                url: typeof window !== "undefined" ? `${window.location.origin}/${entity}/${repo}/issues/${newIssue.id}` : undefined,
              });
    ...
    [router, isLoggedIn, publish, defaultRelays, params, selectedLabels, selectedAssignees, selectedProject, selectedMilestone, submitting, bountyAmount, bountyWithdrawId, bountyLnurl, bountyWithdrawUrl, bountyInvoice, bountyPaymentHash, currentUserPubkey]
  );
                    ...
                    <div className="p-2 space-y-2">
                      ...
                      {currentUserPubkey && (
                        <Button
                          ...
                          disabled={selectedAssignees.includes(currentUserPubkey)}
                        >
                          Assign yourself
                        </Button>
                      )}
                      {selectedAssignees.length > 0 && (
                        <div className="border-t border-gray-700 pt-2 mt-2">
                          <p className="text-xs text-gray-400 mb-2">Assigned ({selectedAssignees.length}/10):</p>
                          <div className="space-y-1 max-h-32 overflow-y-auto">
                            {selectedAssignees.map((pubkey) => {
                              ...
                              return (
                                <div key={pubkey} className="flex items-center justify-between text-xs bg-gray-800 p-1.5 rounded group" title={npub ? `npub: ${npub}` : `pubkey: ${pubkey}`}>
                      ...
                      <>
                        ...
                        <div className="p-2 space-y-2">
                          ...
                          <div className="space-y-1 max-h-40 overflow-y-auto">
                            {repoContributors
                              .filter(c => !selectedAssignees.includes(c.pubkey))
                              .map((contributor) => {
                      ...
                      <DropdownMenuGroup>
                        {availableProjects.map((project) => (
                          <DropdownMenuItem
                            ...
                            className={selectedProject === project.id ? "bg-purple-900/50" : ""}
                          >
                            {selectedProject === project.id && <Check className="mr-2 h-4 w-4" />}
                            {project.name}
                          </DropdownMenuItem>
                        ))}
                      </DropdownMenuGroup>
          ...
          <div className="flex flex-col w-full p-2">
                      ...
                      <DropdownMenuGroup>
                        {availableMilestones.map((milestone) => (
                          <DropdownMenuItem
                            ...
                            className={selectedMilestone === milestone.id ? "bg-purple-900/50" : ""}
                          >
                            {selectedMilestone === milestone.id && <Check className="mr-2 h-4 w-4" />}
                            {milestone.name}
                          </DropdownMenuItem>
                        ))}
                      </DropdownMenuGroup>
            ...
            <div className="mt-1">
              {selectedMilestone ? (
                <div className="flex items-center gap-1">
                  <Badge variant="outline" className="border-purple-700 text-purple-400 bg-purple-900/20">
                    {availableMilestones.find(m => m.id === selectedMilestone)?.name}
                    <button
                      type="button"
          ...
          <div className="flex flex-col w-full p-2">
            ...
            <div className="mt-1">
              <p className="text-xs text-gray-400 mb-2">
                Add a bounty to motivate developers to fix this issue. The bounty will be paid from your sending wallet when the issue is resolved.
              </p>
              <BountyButton
                ...
                repoEntity={typeof params?.entity === "string" ? params.entity : undefined}
                repoName={typeof params?.repo === "string" ? params.repo : undefined}
                onBountyCreated={(withdrawId: string, lnurl: string, withdrawUrl: string, amount: number) => {
              ...
              {bountyAmount > 0 && (
                <div className="mt-2 p-2 bg-green-900/20 border border-green-700 rounded text-sm text-green-400">
                  💰 {bountyAmount.toLocaleString()} sats bounty added (paid via LNURL-withdraw)
                </div>
              )}
            </div>
          </div>
