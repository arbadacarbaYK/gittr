...
import { Search, File, Filter } from "lucide-react";
      ...
      try {
        ...
        if (repo && (!repo.files || repo.files.length === 0) && repo.sourceUrl) {
          try {
            ...
            if (response.ok) {
              const data = (await response.json()) as { files?: RepoFileEntry[] };
  ...
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      for (const file of filesToSearch) {
        ...
        if (fileName.toLowerCase() === normalizedQuery || filePath === normalizedQuery) {
        ...
        if (fileName.toLowerCase().includes(normalizedQuery) || filePath.includes(normalizedQuery)) {
          const matchIndexInName = fileName.toLowerCase().indexOf(normalizedQuery);
          const matchIndexInPath = filePath.indexOf(normalizedQuery);
          const highlightIndex = matchIndexInName >= 0 ? matchIndexInName : matchIndexInPath;
      ...
      for (const file of filesToSearch) {
        ...
        try {
          ...
          if (overrideContent) {
            ...
            try {
              const githubMatch = repoData.sourceUrl.match(/github\.com\/([^\/]+)\/([^\/]+)/);
              if (githubMatch) {
                ...
                const response = await fetch(
                  `https://api.github.com/repos/${owner}/${repoName}/contents/${encodeURIComponent(file.path)}?ref=${branch}`,
      ...
      {loading && query && (
        <div className="border border-gray-700 rounded p-8 text-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-gray-500">This may take a moment for large repositories.</p>
          </div>
        </div>
      )}
                ...
                <table className="w-full font-mono text-sm">
                  <tbody>
                    {result.matches.map((match, matchIdx) => (
                      <tr key={matchIdx} className="hover:bg-gray-800/50 border-b border-gray-800">
