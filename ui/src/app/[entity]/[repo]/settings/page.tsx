  ...
  useEffect(() => {
    ...
    try {
      ...
      if (repoData) {
        ...
        // Load repo links (drop only stale invented github.io from older builds)
        ...
        if (
          JSON.stringify(repoData.links || []) !== JSON.stringify(cleanedLinks)
        ) {
          ...
          if (idx >= 0 && all[idx]) {
            if (cleanedLinks.length > 0) {
              all[idx]!.links = cleanedLinks;
            } else {
              delete all[idx]!.links;
            }
            saveStoredRepos(all);
          }
        }
