...
export function checkRepoFiles(
  ...
  const matchingRepos = allRepos.filter((r: any) => {
    ...
    const entityMatches =
      ...
      (repoEntity &&
        entity &&
        repoEntity.toLowerCase() === entity.toLowerCase());

    // Check repo name match (try all variations)
    const repoMatches = possibleRepos.some(
      (possibleRepo) =>
  ...
  console.log(
    `📦 Found ${matchingRepos.length} matching repo(s) in localStorage:`,
    matchingRepos.map((r: any) => ({
      entity: r.entity,
      repo: r.repo || r.slug,
      name: r.name,
    ...
    if (stored) {
      try {
        ...
        if (Array.isArray(files) && files.length > 0) {
          console.log(
            `✅ Found ${files.length} files in separate storage key: ${filesKey}`
          );
          return {
            found: true,
            key: filesKey,
  ...
  console.log(
    `📋 All gittr_files keys in localStorage (${allFilesKeys.length} total):`,
    allFilesKeys
  );
