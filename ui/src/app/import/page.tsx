...
function normalizeGithubUrl(input: string): string {
  ...
  if (!url.includes('/') && !url.includes('github.com') && !url.includes('http://') && !url.includes('https://')) {
  ...
  try {
    ...
    if (urlObj.hostname === 'github.com' || urlObj.hostname.includes('github.com')) {
...
export default function ImportPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (importAll && repos.length > 0 && !importing && isLoggedIn && selectedRepos.size === 0) {
          ...
          const importAllRepos = async () => {
            ...
            try {
              if (!pubkey || typeof pubkey !== "string") {
                setStatus("Error: Must be logged in with Nostr key to import repositories");
                setImporting(false);
                return;
              }
              ...
              for (const fullName of allRepos) {
                ...
                const existingMatch = existingRepos.find(
                  (r: any) => {
                    // Match by entity AND repo slug (same user, same repo name)
                    const found = findRepoByEntityAndName([r], entityInfo.entitySlug, repoSlug);
                    if (found !== undefined) return true;
                    
                    // Match by ownerPubkey AND repo slug (catches duplicates even if entity format changed)
                    if (pubkey && r.ownerPubkey && r.ownerPubkey.toLowerCase() === pubkey.toLowerCase()) {
                    ...
                    if (r.entity === entityInfo.entitySlug && (r.sourceUrl === repo.html_url || r.forkedFrom === repo.html_url)) {
                      return true;
                    }
                    return false;
                  }
                ...
                try {
                  ...
                  let fileCount = 0;
                  if (importData.files && Array.isArray(importData.files) && importData.files.length > 0) {
                    fileCount = importData.files.length;
                    try {
                      const filesKey = getRepoStorageKey("gittr_files", entityInfo.entitySlug, repoSlug);
                      localStorage.setItem(filesKey, JSON.stringify(importData.files));
                    } catch (e) {
                      console.error(`❌ [Import] Failed to save files separately for ${fullName}:`, e);
                      fileCount = importData.files.length;
                    }
                  }
                  ...
                  if (!rec.contributors || rec.contributors.length === 0 || !rec.contributors.some((c: any) => c.pubkey === pubkey)) {
                    if (pubkey) {
                      if (!rec.contributors) rec.contributors = [];
                      rec.contributors = rec.contributors.filter((c: any) => !c.pubkey || c.pubkey !== pubkey);
                      rec.contributors.unshift({ pubkey, name: entityInfo.displayName, weight: 100 });
                    }
                  }
                  ...
                  if (importData.issues && Array.isArray(importData.issues) && importData.issues.length > 0) {
                    try {
                      const issuesKey = getRepoStorageKey("gittr_issues", entityInfo.entitySlug, repoSlug);
                      ...
                      console.error(`❌ [Import] Failed to save issues for ${fullName}:`, e);
                    }
                  }
                  
                  if (importData.pulls && Array.isArray(importData.pulls) && importData.pulls.length > 0) {
                    try {
                      const pullsKey = getRepoStorageKey("gittr_prs", entityInfo.entitySlug, repoSlug);
                      ...
                      console.error(`❌ [Import] Failed to save pull requests for ${fullName}:`, e);
                    }
                  }
                  
                  if (importData.commits && Array.isArray(importData.commits) && importData.commits.length > 0) {
                    try {
                      const commitsKey = getRepoStorageKey("gittr_commits", entityInfo.entitySlug, repoSlug);
                      const formattedCommits = importData.commits.map((commit: any) => ({
                      ...
                      console.error(`❌ [Import] Failed to save commits for ${fullName}:`, e);
                    }
                  }
                    ...
                    try {
                      ...
                      if (privateKey) {
                        ...
                        const gitServerUrl = process.env.NEXT_PUBLIC_GIT_SERVER_URL || 
                          (typeof window !== 'undefined' ? `${window.location.protocol}//${window.location.host}` : 'https://gittr.space');
                        ...
                        try {
                          if (!publish || !subscribe) {
                            console.error(`❌ [Import] Cannot publish ${fullName}: publish or subscribe is undefined`);
                            continue;
                          }
                          ...
                          console.error(`Failed to publish ${fullName} with confirmation:`, pubError);
                        }
                      }
                    } catch (error: any) {
                      console.error(`Failed to publish ${fullName} to Nostr:`, error);
                    }
                  }
                } catch (error: any) {
              ...
              setStatus(
                `Imported ${importedCount} repositories${skippedCount > 0 ? `, ${skippedCount} skipped (already exist)` : ""}`
              );
              setTimeout(() => {
  ...
  }, [repos.length, importing, isLoggedIn, selectedRepos.size, pubkey, userName, publish, defaultRelays, router]);
  ...
  const fetchUserRepos = useCallback(async () => {
    ...
    try {
      ...
      if (trimmed.match(/^[a-zA-Z0-9_-]+$/) && !trimmed.includes('.') && !trimmed.includes('/') && !trimmed.includes('http')) {
        username = trimmed;
        ...
        try {
          ...
          if (pathParts.length >= 2 && urlObj.hostname.includes('github.com')) {
            ...
            console.log("Import: Detected specific repo URL:", { repoUsername, repoNameFromUrl });
            ...
            if (!repoResponse.ok && repoResponse.status === 404) {
              console.log("Import: Exact case not found, fetching all repos for case-insensitive match");
              ...
              if (userReposResponse.ok) {
                ...
                const matchedRepo = allRepos.find(
                  (r) => r.name.toLowerCase() === repoNameFromUrl.toLowerCase() && !r.private
                );

                if (matchedRepo) {
                  console.log(`Import: Found case-insensitive match: "${matchedRepo.name}" (requested: "${repoNameFromUrl}")`);
                  ...
                  setStatus(`Repository "${repoUsername}/${repoNameFromUrl}" not found (case-insensitive search also failed)`);
                  setLoading(false);
                  return;
                }
              }
            ...
            if (!repoResponse.ok) {
              if (repoResponse.status === 404) {
                setStatus(`Repository "${repoUsername}/${repoNameFromUrl}" not found on GitHub`);
              } else {
                setStatus(`Failed to fetch repository: ${repoResponse.statusText}`);
              }
              setLoading(false);
              return;
            }
            ...
            if (repoData.private) {
              setStatus(`Repository "${repoData.name}" is private and cannot be imported`);
              setLoading(false);
              return;
            }
      ...
      if (!username || username.length === 0) {
        setStatus(`Could not extract username from "${githubUrl}". Please enter a valid GitHub username (e.g., arbadacarbaYK) or profile URL.`);
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    ...
    if (!githubUrl.trim() || loading || githubUrl.trim() === lastFetchedUrlRef.current) {
      return;
    }
  ...
  async function importSelected() {
    ...
    try {
      ...
      for (const fullName of selectedRepos) {
        ...
        if (existingMatch) {
          console.log(`Skipping ${fullName} - already exists for this user:`, {
            ...
            matchReason: existingMatch.entity === entityInfo.entitySlug && existingMatch.repo === repoSlug 
        ...
        try {
          ...
          if (!importData.files || !Array.isArray(importData.files) || importData.files.length === 0) {
            console.error(`❌ [Import] No files returned for ${fullName}`, importData);
          }
          ...
          if (pubkey) {
            ...
            finalContributors = finalContributors.filter((c: any) => !c.pubkey || c.pubkey.toLowerCase() !== pubkey.toLowerCase());
            // Add owner at the beginning with weight 100
          ...
          const links: Array<{type: "docs" | "discord" | "slack" | "youtube" | "twitter" | "github" | "other"; url: string; label?: string}> = [];
          ...
          if (importData.files && Array.isArray(importData.files) && importData.files.length > 0) {
            fileCount = importData.files.length;
            try {
              const filesKey = getRepoStorageKey("gittr_files", entityNpub, repoSlug);
              localStorage.setItem(filesKey, JSON.stringify(importData.files));
            } catch (e) {
              console.error(`❌ [Import] Failed to save files separately for ${fullName}:`, e);
          ...
          if (importData.issues && Array.isArray(importData.issues) && importData.issues.length > 0) {
            try {
              const issuesKey = getRepoStorageKey("gittr_issues", entityNpub, repoSlug);
          ...
          if (importData.pulls && Array.isArray(importData.pulls) && importData.pulls.length > 0) {
            try {
              const pullsKey = getRepoStorageKey("gittr_prs", entityNpub, repoSlug);
          ...
          if (importData.commits && Array.isArray(importData.commits) && importData.commits.length > 0) {
            try {
              const commitsKey = getRepoStorageKey("gittr_commits", entityNpub, repoSlug);
          ...
          if (repoSlug.toLowerCase() === "tides") {
            console.debug("🌊 [Import] Tides repo snapshot", {
              ...
              hasContributors: !!(rec.contributors && Array.isArray(rec.contributors)),
              contributorsCount: rec.contributors?.length || 0,
              ownerContributor: rec.contributors?.find((c: any) => c.weight === 100),
              repo: rec.repo,
              slug: rec.slug,
              name: rec.name,
            });
            ...
            try {
              ...
              if (privateKey) {
                ...
                try {
                  ...
                  console.error(`❌ [Import] Failed to publish ${fullName} with confirmation:`, pubError);
                  // Mark repo as push_failed but keep it in localStorage
                  try {
                    const repos = JSON.parse(localStorage.getItem("gittr_repos") || "[]");
                    const repoIndex = repos.findIndex((r: any) => 
                      (r.slug === repoSlug || r.repo === repoSlug) && r.entity === rec.entity
                    );
                    if (repoIndex >= 0) {
                      repos[repoIndex] = { ...repos[repoIndex], status: "push_failed", pushError: pubError.message };
              ...
              console.error(`❌ [Import] Failed to publish ${fullName} to Nostr:`, error);
              // Mark repo as push_failed but keep it in localStorage
              try {
                const repos = JSON.parse(localStorage.getItem("gittr_repos") || "[]");
                const repoIndex = repos.findIndex((r: any) => 
                ...
                if (repoIndex >= 0) {
                  repos[repoIndex] = { ...repos[repoIndex], status: "push_failed", pushError: error.message };
                  localStorage.setItem("gittr_repos", JSON.stringify(repos));
                }
              } catch (e) {
                console.error(`Failed to mark ${fullName} as push_failed:`, e);
      ...
      console.log(`💾 [Import] Saving ${existingRepos.length} repos to localStorage (${importedCount} imported, ${skippedCount} skipped)`);
      ...
      if (cleanedRepos.length < existingRepos.length) {
        console.log(`🧹 [Import] Cleaned up ${existingRepos.length - cleanedRepos.length} old repos (older than 90 days)`);
      }
      ...
      try {
        ...
        if (e.name === 'QuotaExceededError' || e.message?.includes('quota')) {
          console.error(`❌ [Import] localStorage quota exceeded! Attempting cleanup...`);
          ...
          try {
            localStorage.setItem("gittr_repos", JSON.stringify(aggressiveCleanup));
            console.log(`🧹 [Import] Aggressive cleanup: removed ${cleanedRepos.length - aggressiveCleanup.length} repos (older than 30 days)`);
            alert(`⚠️ localStorage quota exceeded. Cleaned up old repos. ${aggressiveCleanup.length} repos remaining.`);
          } catch (e2: any) {
            console.error(`❌ [Import] Still quota exceeded after cleanup!`, e2);
            alert(`❌ Error: localStorage is full. Please clear browser data or remove some repos manually.`);
            throw e2;
          }
        } else {
          throw e;
      ...
      const savedRepos = JSON.parse(localStorage.getItem("gittr_repos") || "[]");
      ...
      if (tidesInSaved) {
        console.debug("🌊 [Import] Tides repo verified in localStorage", {
          ...
          hasContributors: !!(tidesInSaved.contributors && Array.isArray(tidesInSaved.contributors)),
          contributorsCount: tidesInSaved.contributors?.length || 0,
          ownerContributor: tidesInSaved.contributors?.find((c: any) => c.weight === 100),
          repo: tidesInSaved.repo,
          slug: tidesInSaved.slug,
          name: tidesInSaved.name,
        });
      } else {
        console.error("❌ [Import] ERROR: tides repo NOT found in localStorage after save!");
        console.error("All saved repos:", savedRepos.map((r: any) => ({
          repo: r.repo,
          slug: r.slug,
          name: r.name,
          entity: r.entity,
    ...
    <div className="container mx-auto max-w-[95%] xl:max-w-[90%] 2xl:max-w-[85%] p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-1">
          Enter a GitHub username or profile URL. https:// will be added automatically.
        </p>
      </div>

      {status && (
        <div className={`mb-4 p-3 rounded ${status.includes("Error") || status.includes("Failed") ? "bg-red-900/20 border border-red-500" : "bg-[#22262C] border border-[#383B42]"}`}>
      ...
      {repos.length > 0 && (
        <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {repo.private && (
                          <span className="text-xs bg-yellow-900/30 text-yellow-400 px-1 rounded">Private</span>
                        )}
                      </div>
                      {repo.description && (
          ...
          <div className="flex gap-2">
            ...
            >
              {importing ? `Importing ${selectedRepos.size} repositories...` : `Import ${selectedRepos.size} Selected Repositories`}
            </button>
            <button
            ...
            {showImportAllConfirm && (
              <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={() => setShowImportAllConfirm(false)}>
                <div className="bg-[#0E1116] border border-[#383B42] rounded-lg p-6 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                  <h2 className="text-xl font-bold mb-4 text-yellow-400">⚠️ Import All Repositories</h2>
                  
                  <div className="space-y-4 mb-6">
                    <div className="bg-yellow-900/20 border border-yellow-600/50 rounded p-4">
                      <p className="font-semibold text-yellow-300 mb-2">What will be imported:</p>
                      <ul className="list-disc list-inside space-y-1 text-sm text-yellow-200/90">
                        <li><strong>{repos.length} repositories</strong> from {githubUrl || "GitHub"}</li>
                        <li>All files, issues, PRs, and commits for each repository</li>
                        <li>All data will be stored in your browser's localStorage</li>
                      </ul>
                    </div>
                    
                    <div className="bg-blue-900/20 border border-blue-600/50 rounded p-4">
                      <p className="font-semibold text-blue-300 mb-2">⚠️ Important considerations:</p>
                      <ul className="list-disc list-inside space-y-1 text-sm text-blue-200/90">
                        ...
                        <li>Large imports may cause browser slowdown or crashes</li>
                        <li>Closing the browser during import can corrupt localStorage data</li>
                        <li>You should be logged in with your Nostr key</li>
                      </ul>
                    </div>
                    
                    <div className="bg-red-900/20 border border-red-600/50 rounded p-4">
                      <p className="font-semibold text-red-300 mb-2">⚠️ Warning:</p>
                      <p className="text-sm text-red-200/90">
                        If you accidentally click this or close the browser during import, you may end up with corrupted data in localStorage. Make sure you have time to complete the import process.
                      </p>
                    </div>
                  </div>
