...
function ExplorePageContent() {
  ...
  const metadataKey = useMemo(() => ownerPubkeys.join(","), [ownerPubkeys]);
  const lastMetadataKeyRef = useRef<string | null>(null);

  // Subscribe to metadata for the current batch of owner pubkeys
  useEffect(() => {
    console.log('[Explore Metadata] effect run', {
      subscribe: !!subscribe,
      defaultRelays: defaultRelays?.length || 0,
      ownerPubkeysCount: ownerPubkeys.length,
      metadataKey,
      lastMetadataKey: lastMetadataKeyRef.current,
    });

    if (!subscribe || !defaultRelays) {
      return;
    }

    if (ownerPubkeys.length === 0) {
      lastMetadataKeyRef.current = null;
      setIsLoadingMetadata(false);
      return;
    }

    if (lastMetadataKeyRef.current === metadataKey) {
      // Already handling this batch
      return;
    }

    lastMetadataKeyRef.current = metadataKey;

    const validPubkeys = ownerPubkeys.filter(p => /^[0-9a-f]{64}$/i.test(p));
    if (validPubkeys.length === 0) {
      console.warn("⚠️ [Explore Metadata] No valid pubkeys to subscribe for!", ownerPubkeys);
      setIsLoadingMetadata(false);
      return;
    }

    console.log("🚀 [Explore Metadata] Subscribing for metadata batch", {
      count: validPubkeys.length,
      sample: validPubkeys.slice(0, 3).map(p => p.slice(0, 8)),
      metadataKey,
      cachedCount: Object.keys(ownerMetadata).length
    });

    // Check if we already have cached metadata for any of these pubkeys
    const missingPubkeys = validPubkeys.filter(p => !ownerMetadata[p]);
    if (missingPubkeys.length < validPubkeys.length) {
      console.log("✅ [Explore Metadata] Using cached metadata for some pubkeys", {
        total: validPubkeys.length,
        cached: validPubkeys.length - missingPubkeys.length,
        missing: missingPubkeys.length
      });
    }
    ...
    setIsLoadingMetadata(missingPubkeys.length > 0);

    const timeout = setTimeout(() => {
      if (isActive) {
        console.warn("⏳ [Explore Metadata] Timeout waiting for metadata events", {
          count: validPubkeys.length,
          metadataKey
        });
        setIsLoadingMetadata(false);
    ...
    try {
      unsub = subscribe(
        ...
        (event) => {
          ...
          try {
            ...
            metadataUpdateQueueRef.current.set(event.pubkey, newMetadata);
            
            // Batch updates: only apply every 100ms to prevent render loops
            if (!metadataUpdateTimeoutRef.current) {
              metadataUpdateTimeoutRef.current = setTimeout(() => {
                setOwnerMetadata(prev => {
                  let updated = { ...prev };
                  let hasChanges = false;
                  
                  // Apply all queued updates
                  for (const [pubkey, metadata] of metadataUpdateQueueRef.current.entries()) {
                    const existing = prev[pubkey] as (Metadata & { created_at?: number }) | undefined;
                    // Only update if newer
                    if (!existing || !existing.created_at || metadata.created_at > existing.created_at) {
                      updated[pubkey] = metadata;
                      hasChanges = true;
                    }
                  }
                  
                  // Clear queue
                  metadataUpdateQueueRef.current.clear();
            ...
            console.error("❌ [Explore Metadata] Failed to parse metadata", err, {
        ...
        undefined,
        (events, relayURL) => {
          console.log("✅ [Explore Metadata] EOSE", {
            relay: relayURL,
      ...
      console.error("❌ [Explore Metadata] Failed to subscribe for metadata", error);
    ...
    return () => {
      ...
      if (metadataUpdateQueueRef.current.size > 0) {
        setOwnerMetadata(prev => {
          let updated = { ...prev };
          for (const [pubkey, metadata] of metadataUpdateQueueRef.current.entries()) {
            const existing = prev[pubkey] as (Metadata & { created_at?: number }) | undefined;
      ...
      if (unsub) {
        console.log("🔄 [Explore Metadata] Unsubscribing metadata batch", {
          metadataKey,
          count: validPubkeys.length
        });
        unsub();
  ...
  }, [subscribe, defaultRelays, metadataKey]); // Only depend on metadataKey, not ownerPubkeys.length
