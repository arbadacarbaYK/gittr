 ...
 */
export function resolveEntityToPubkey(entity: string | undefined, repo?: any): string | null {
...
export function getRepoOwnerPubkey(repo: any, entity?: string): string | null {
  ...
  if (repo.contributors && Array.isArray(repo.contributors)) {
    const owner = repo.contributors.find((c: any) => 
      c.weight === 100 && 
      c.pubkey && 
...
export function getUserMetadata(
  ...
  if (metadataMap[normalized]) {
    ...
    if (meta.identities && Array.isArray(meta.identities) && meta.identities.length > 0) {
      console.log(`✅ [getUserMetadata] Found ${meta.identities.length} identities for ${normalized.slice(0, 8)}`);
    }
    return meta;
  }
  ...
  if (metadataMap[pubkey]) {
    ...
    if (meta.identities && Array.isArray(meta.identities) && meta.identities.length > 0) {
      console.log(`✅ [getUserMetadata] Found ${meta.identities.length} identities for ${pubkey.slice(0, 8)} (original case)`);
    }
    return meta;
  }
  
  // Strategy 3: Case-insensitive match (find key that matches when lowercased)
  const matchingKey = Object.keys(metadataMap).find(k => k.toLowerCase() === normalized);
  if (matchingKey && metadataMap[matchingKey]) {
    const meta = metadataMap[matchingKey];
    if (meta.identities && Array.isArray(meta.identities) && meta.identities.length > 0) {
      console.log(`✅ [getUserMetadata] Found ${meta.identities.length} identities for ${normalized.slice(0, 8)} (case-insensitive match)`);
    }
    return meta;
  }
  
  // Strategy 4: Partial match (first 8 chars) as last resort
  const partialMatch = Object.keys(metadataMap).find(k => 
    k.toLowerCase().slice(0, 8) === normalized.slice(0, 8) && 
    k.length === 64
  );
  if (partialMatch && metadataMap[partialMatch]) {
    const meta = metadataMap[partialMatch];
    if (meta.identities && Array.isArray(meta.identities) && meta.identities.length > 0) {
      console.log(`✅ [getUserMetadata] Found ${meta.identities.length} identities for ${normalized.slice(0, 8)} (partial match)`);
    }
    return meta;
  }
  
  console.log(`⚠️ [getUserMetadata] No metadata found for ${normalized.slice(0, 8)} (available keys: ${Object.keys(metadataMap).slice(0, 5).join(", ")})`);
  return {};
}
