...
export async function findCorruptedEventPublishers(
  ...
  const results = new Map<
    string,
    { pubkey: string; entity?: string; content?: any }
  >();
  ...
  return new Promise((resolve) => {
    ...
    const onEose = () => {
      ...
      if (results.size < CORRUPTED_TIDES_EVENT_IDS.length) {
        console.log(
          `⚠️  Only found ${results.size} of ${CORRUPTED_TIDES_EVENT_IDS.length} corrupted events`
        );
      }
    };
...
if (typeof window !== "undefined") {
  (window as any).findCorruptedEventPublishers = async () => {
    ...
    console.log(
      "💡 Example: findCorruptedEventPublishers(['wss://relay.example.com'])"
    );
  };
}
