 ...
 */
export function formatPaymentMessage(username?: string | null, maxLength: number = 160): string {
  ...
  if (baseMessage.length > maxLength) {
    ...
    const truncatedUsername = displayName.substring(0, Math.max(1, maxUsernameLength));
