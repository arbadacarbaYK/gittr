...
export async function getNWCBalance(nwcUri: string): Promise<NWCBalanceResult> {
  ...
  return new Promise<NWCBalanceResult>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!responseReceived) {
        ws.close();
        reject(new Error('NWC balance request timed out. This wallet may not support get_balance method. Many wallets only support pay_invoice.'));
      }
    ...
    ws.onmessage = async (message) => {
      try {
        ...
        // Handle OK response
        if (Array.isArray(data) && data[0] === 'OK' && data[1] === requestEvent.id) {
          if (data[2] === true) {
        ...
        // Handle response event
          ...
          if (event && event.kind === 23195 && event.pubkey === walletPubkey) {
            ...
            if (responseRequestId !== requestEvent.id) {
              console.log("NWC Balance: Ignoring response - 'e' tag doesn't match request ID:", {
                responseRequestId,
            ...
            if (event.created_at >= requestTimestamp) {
              console.log("NWC Balance: Received response event (matches request ID)");
              responseReceived = true;
              
              try {
                // Decrypt response
                const decrypted = await nip04.decrypt(sk, walletPubkey, event.content);
                ...
                if (response.error) {
                  ...
                  if (errorCode === -32601 || errorMessage.toLowerCase().includes('method') || errorMessage.toLowerCase().includes('not supported')) {
                    reject(new Error(`get_balance method not supported by this wallet. Many wallets only support pay_invoice.`));
                  } else {
                ...
                clearTimeout(timeout);
                ws.close();
                reject(new Error(`Failed to decrypt response: ${decryptError.message}`));
              }
            }
          }
        }
