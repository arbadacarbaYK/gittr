...
export function migrateEntityToPubkey(
  ...
  if (!currentUserPubkey || typeof currentUserPubkey !== "string") {
    ...
    console.log(
      "⚠️ [Migration] Running migration without currentUserPubkey - will migrate all repos with ownerPubkey"
    );
  }
  ...
  try {
    ...
    const entityNpub =
      currentUserPubkey && typeof currentUserPubkey === "string"
        ? nip19.npubEncode(currentUserPubkey)
        : null;
    const displayName =
      currentUserPubkey &&
      currentUserName &&
      currentUserName !== "Anonymous Nostrich"
        ? currentUserName.trim()
        : entityNpub
        ? entityNpub.slice(0, 12)
        : null; // Shortened npub for display
    ...
    const migratedRepos = repos.map((r) => {
      ...
      const isGitHubUsername =
        ...
        (r.entity.includes("-") ||
          r.entity.includes("_") ||
          !/^[0-9a-f]+$/i.test(r.entity));

      // Check if entity is an 8-char pubkey prefix (old format we want to migrate)
      const is8CharPrefix =
        r.entity && r.entity.length === 8 && /^[0-9a-f]{8}$/i.test(r.entity);
      ...
      const entityMatchesOwnerPrefix =
        is8CharPrefix &&
        r.ownerPubkey &&
      ...
      if (
        currentUserPubkey &&
        (isGitHubUsername || is8CharPrefix) &&
        r.ownerPubkey === currentUserPubkey
      ) {
        updated = true;
        return {
          ...r,
      ...
      if (
        ...
        return {
          ...
          entityDisplayName:
            r.entityDisplayName || repoEntityNpub.slice(0, 12) + "...",
      ...
      if (is8CharPrefix && !r.ownerPubkey) {
        ...
        console.warn(
          "⚠️ [Migration] Repo with 8-char entity but no ownerPubkey:",
          {
            entity: r.entity,
            repo: r.repo || r.slug,
            name: r.name,
    ...
    if (updated) {
      try {
        ...
        const migratedCount = migratedRepos.filter(
          (r) =>
            r.entity &&
            (r.entity.startsWith("npub") || /^[0-9a-f]{64}$/i.test(r.entity))
        ).length;
        console.log(
          `✅ [Migration] Migrated ${migratedCount} repos to use entity npub format (GRASP protocol standard). Total repos: ${migratedRepos.length}`
        );
        return true;
        ...
        if (
          ...
          console.warn(
            "⚠️ [Migration] Cannot migrate entity to npub: localStorage quota exceeded. Consider cleaning up old data."
          );
          return false;
        }
