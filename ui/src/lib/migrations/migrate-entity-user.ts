 ...
 */
export function migrateEntityUser(userName: string | null, isLoggedIn: boolean): boolean {
  if (!isLoggedIn || !userName || userName === "Anonymous Nostrich" || userName.trim() === "") {
  ...
  try {
    ...
    if (updated) {
      try {
        ...
        console.log(`Migrated ${migratedRepos.filter(r => r.entity === entitySlug).length} repos to use entity "${entitySlug}"`);
        return true;
