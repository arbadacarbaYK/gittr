...
export function validatePaymentAmount(amount: number | string): {
  valid: boolean;
  error?: string;
} {
...
export function validateUrl(
  url: string,
  allowedProtocols: string[] = ["https:", "http:", "wss:", "ws:"]
...
export function validateLightningAddress(address: string): {
  ...
  if (!lud16Regex.test(address)) {
    return {
      valid: false,
      error: "Lightning address must be in format: name@domain",
    };
  }
...
// Validate LNURL
export function validateLNURL(lnurl: string): {
  valid: boolean;
  error?: string;
} {
...
// Validate NWC string
export function validateNWCString(nwc: string): {
  valid: boolean;
  error?: string;
} {
  ...
  if (!nwc.startsWith("nostr+walletconnect://")) {
    return {
      valid: false,
      error: "NWC string must start with 'nostr+walletconnect://'",
    };
  }
