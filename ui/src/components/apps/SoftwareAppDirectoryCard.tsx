...
/** Compact so APK + Repo + Releases + Web stay on one line. */
export const SOFTWARE_APP_CARD_ACTION_CLASS = "h-8 shrink-0 px-2 text-xs";
...
export function SoftwareAppDirectoryCard({
  ...
  return (
    <article className={cn(DIRECTORY_TILE_ARTICLE_CLASS, "flex-col")}>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-1.5 border-t border-[#383B42]/60 pt-4">
        {footer}
      </div>
    </article>
  );
...
export function SoftwareAppProfileFooter({ app }: { app: ParsedSoftwareApp }) {
  ...
  return (
    <>
      <SoftwareAppSourceButtons app={app} webVariant="default" />
