      ...
      try {
        ...
        if (isUsernameQuery) {
          ...
          const topicMatches = repos.filter((r: any) => {
            ...
            return topics.some(
              (topic: string) =>
                topic.includes(normalized) || normalized.includes(topic)
            );
          });
          ...
          const userRepos = repos
            .filter((r: any) => {
              ...
              return (
                ...
                (entityDisplayName &&
                  entityDisplayName.toLowerCase().includes(normalized)) ||
                (entity &&
                  normalized.length >= 4 &&
                  entity.toLowerCase().startsWith(normalized))
              );
            })
          ...
          if (userRepos.length > 0) {
            ...
            if (ownerPubkey && /^[0-9a-f]{64}$/i.test(ownerPubkey)) {
              try {
                ...
                console.error("⚠️ [Search] Failed to encode npub:", {
                  ownerPubkey,
                  error,
                });
            ...
            window.location.href = `/explore?user=${encodeURIComponent(
              entityForUrl
