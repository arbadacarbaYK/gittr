...
export function CodeViewer({
  ...
  useEffect(() => {
    ...
    if (
      currentFileFromUrl &&
      currentFileFromUrl !== lastFileFromUrlRef.current &&
      lastFileFromUrlRef.current
    ) {
  ...
  const handleShareAsSnippet = async () => {
    ...
    try {
      ...
      if (result.confirmed) {
        ...
        const toast = document.createElement("div");
        toast.className =
          "fixed bottom-4 right-4 bg-green-800 border border-green-600 text-white px-4 py-2 rounded shadow-lg z-50";
        toast.textContent = `Snippet shared! Event ID: ${result.eventId.substring(
          0,
          16
        )}...`;
        document.body.appendChild(toast);
    ...
    <div ref={containerRef} className="relative">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <strong className="text-blue-200 text-sm">
                Step 1: Click the first line
              </strong>
              <p className="text-xs text-blue-300 mt-1">
                Click on the line number or code where you want your selection
                to start
              </p>
            </div>
          </div>
      ...
      {rangeMode && selectionStart !== null && (
        <div className="mt-3 p-3 bg-green-900/40 border-2 border-green-500 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <div>
              <strong className="text-green-200 text-sm">
                Step 2: Click the last line
              </strong>
              <p className="text-xs text-green-300 mt-1">
                Click on the line where you want your selection to end
                (currently starting at line {selectionStart})
              </p>
            </div>
          </div>
        </div>
      ...
      {!rangeMode && (
        <div className="mt-3 p-2 bg-gray-700/50 border border-gray-600 rounded text-xs text-gray-300">
          💡 <strong>Tip:</strong> You can also <strong>click and drag</strong>{" "}
          across lines to select a range instantly, or use "Select Range" for
          two-click selection
        </div>
      )}
      {!rangeMode &&
        ...
        selectedLines.end === selectedLines.start && (
          <div className="mt-3 p-2 bg-yellow-900/30 border border-yellow-600 rounded text-xs text-yellow-200">
            💡 <strong>Want to select multiple lines?</strong> Click "Select
            Range" above, then click the first and last lines you want to share.
          </div>
        )}
          ...
          <div className="bg-[#171B21] border border-[#383B42] rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="text-xs text-gray-400">
                <p>
                  Language:{" "}
                  {getLanguageFromExtension(filePath) || "auto-detect"}
                </p>
                <p>File: {filePath}</p>
                {selectedLines ? (
                  <p>
