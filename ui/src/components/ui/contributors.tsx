...
export function Contributors({ contributors = [] }: ContributorsProps) {
  ...
  if (sanitizedContributors.length === 0) {
    return <div className="text-gray-400 text-sm">No contributors yet</div>;
  }
    ...
    <div className="flex flex-wrap gap-2">
      {sanitizedContributors.map((contributor, idx) => {
        ...
        if (idx === 0) {
          ...
          const contributorsWithPubkeys = sanitizedContributors.filter(
            (c) => c.pubkey
          );
          const uniquePubkeys = new Set(
            contributorsWithPubkeys.map((c) => c.pubkey)
          );
          ...
          if (contributorsWithPubkeys.length > 1 && uniquePubkeys.size === 1) {
            console.error(
              "❌ [Contributors] Multiple contributors have the SAME pubkey! This is wrong:",
              Array.from(uniquePubkeys)[0]
            );
            // Only log full details if there's a problem
            console.table(
              sanitizedContributors.map((c, i) => ({
                idx: i,
                pubkey: c.pubkey
            ...
            console.debug(
              "ℹ️ [Contributors] No contributors have Nostr pubkeys (they may not have claimed GitHub identities)"
            );
          }
        }
        ...
        const picture =
          nostrData?.picture && nostrData.picture.trim().length > 0
            ? nostrData.picture
        ...
        const name =
          nostrData?.display_name ||
          nostrData?.name ||
          contributor.name ||
          contributor.githubLogin ||
          null; // null = no real name, use platform default
        ...
        const isNpubLike =
          name && (name.startsWith("npub") || name.length > 20);
        const isFallbackName =
          name &&
          (name.startsWith("Contributor ") || name.match(/^[0-9a-f]{8,}$/i)); // pubkey prefix or fallback pattern
        const shouldUseInitials = name && !isNpubLike && !isFallbackName;
        const words = shouldUseInitials ? name.trim().split(/\s+/) : [];
        const firstWord = words[0] || "";
        const lastWord = words[words.length - 1] || "";
        const initials =
          shouldUseInitials && words.length > 1 && firstWord && lastWord
        ...
        if (!contributor.pubkey && !contributor.githubLogin) {
          console.warn(
            "⚠️ [Contributors] Contributor missing pubkey and GitHub login",
            contributor
          );
        } else if (
          contributor.pubkey &&
          (!contributor.pubkey.length ||
            contributor.pubkey.length !== 64 ||
            !/^[0-9a-f]{64}$/i.test(contributor.pubkey))
        ) {
        ...
        const npub =
          contributor.pubkey && contributor.pubkey.length === 64
            ? (() => {
                try {
        ...
        const displayNameForTooltip =
          name ||
          (contributor.pubkey
            ? `User ${contributor.pubkey.slice(0, 8)}`
            : `Contributor ${idx + 1}`);
        ...
        return (
          <Tooltip
            key={contributor.pubkey || `contributor-${idx}`}
            content={tooltip}
          >
            <Link
              href={href}
              ...
              <Avatar
                className={`h-8 w-8 shrink-0 overflow-hidden ${
                  ...
                  (contributor.role === undefined &&
                    (contributor.weight ?? 0) === 100)
                    ? "ring-2 ring-purple-500"
                    : contributor.role === "maintainer" ||
                      (contributor.role === undefined &&
                        (contributor.weight ?? 0) >= 50 &&
                        (contributor.weight ?? 0) < 100)
