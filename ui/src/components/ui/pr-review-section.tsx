...
import { type StoredContributor, type StoredRepo } from "@/lib/repos/storage";
import {
  formatDate24h,
  formatDateTime24h,
  formatTime24h,
...
export function PRReviewSection({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (foundRepo && currentUserPubkey) {
        ...
        const isRepoOwner =
          foundRepoWithContributors.ownerPubkey === currentUserPubkey ||
          foundRepoWithContributors.contributors?.some(
            (c: StoredContributor) =>
        ...
        const isMaintainer = foundRepoWithContributors.contributors?.some(
          (c: StoredContributor) =>
            c.pubkey === currentUserPubkey &&
            (c.role === "maintainer" ||
              (c.role === undefined &&
        ...
        const isRepoContributor = foundRepoWithContributors.contributors?.some(
          (c: StoredContributor) =>
            c.pubkey === currentUserPubkey &&
            (c.role === "contributor" ||
              (c.role === undefined &&
        ...
        setIsContributor(
          isRepoOwner || isMaintainer || isRepoContributor || false
        );
      }
    } catch {
  ...
  const handleSubmitReview = useCallback(() => {
    ...
    const updatedReviews = reviews.filter(
      (r) => r.reviewer !== currentUserPubkey
    );
  ...
  const approvalsFromMergeRightsHolders = reviews.filter((r: any) => {
    ...
    const reviewer = repoData.contributors.find(
  ...
  useEffect(() => {
    ...
    const repoMaintainer = repoData.contributors?.some(
      (c: any) =>
        c.pubkey === currentUserPubkey &&
        (c.role === "maintainer" ||
          (c.role === undefined && c.weight >= 50 && c.weight < 100))
    );
  ...
  const isMaintainerPR =
    isCurrentUserMaintainer && prAuthor === currentUserPubkey;
  const canMergeOwnPR = isOwnerPR || isMaintainerPR;

  const canMerge =
    canMergeOwnPR ||
    requiredApprovals === 0 ||
    (approvalsFromMergeRightsHolders >= requiredApprovals &&
      changeRequests === 0);
    ...
    <div className="border border-gray-700 rounded p-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {allApprovals > 0 && (
            <Badge className="theme-bg-accent-primary">
              ...
              {approvalsFromMergeRightsHolders < allApprovals &&
                ` (${approvalsFromMergeRightsHolders} from owners/maintainers)`}
            </Badge>
          )}
          {changeRequests > 0 && (
            <Badge className="bg-red-600">
              ...
              {changeRequests === 1 ? "request" : "requests"}
            </Badge>
          )}
          {canMerge && (
            <Badge className="theme-bg-accent-primary">
              {canMergeOwnPR
                ? "Owner/Maintainer PR - ready to merge"
                : "Ready to merge"}
            </Badge>
          )}
          {!canMergeOwnPR &&
            requiredApprovals > 0 &&
            requiredApprovals > approvalsFromMergeRightsHolders && (
              <Badge variant="outline" className="text-gray-400">
                {requiredApprovals - approvalsFromMergeRightsHolders} more{" "}
                {requiredApprovals - approvalsFromMergeRightsHolders === 1
                  ? "approval"
                  : "approvals"}{" "}
                required from owners/maintainers
              </Badge>
            )}
        ...
        <div className="space-y-3 mb-4">
          {reviews.map((review) => {
            ...
            return (
              <div
                key={review.id}
                className="flex gap-3 p-3 bg-gray-800 rounded"
              >
                <Avatar className="h-8 w-8 shrink-0 overflow-hidden">
                  {review.reviewerPicture &&
                  review.reviewerPicture.startsWith("http") ? (
                    <AvatarImage
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium">
                      {review.reviewerName || review.reviewer.slice(0, 8)}
                    </span>
      ...
      {!userReview && (isContributor || isOwner) ? (
            ...
            <div className="mb-3">
              ...
              <div className="flex gap-2 mb-3">
                <Button
                  ...
                  className={
                    reviewState === "APPROVED" ? "theme-bg-accent-primary" : ""
                  }
                >
                ...
                <Button
                  size="sm"
                  variant={
                    reviewState === "CHANGES_REQUESTED" ? "default" : "outline"
                  }
              ...
              <div className="flex gap-2">
                <Button onClick={handleSubmitReview} variant="default">
                  Submit review
                </Button>
                <Button
                  variant="outline"
        ...
        <div className="text-sm text-gray-500 bg-gray-800/50 p-2 rounded">
          Only repository owners, maintainers, and contributors can approve or
