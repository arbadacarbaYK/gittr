...
export default function DistributeZaps({
  entity,
  repo,
  contributors = [],
}: DistributeZapsProps) {
  ...
  const pubkeys = useMemo(
    () => contributors.map((c) => c.pubkey).filter((p): p is string => !!p),
    [contributors]
  );
  ...
  const totalAmount = useMemo(
    () => rows.reduce((sum, r) => sum + (r.enabled ? r.amount : 0), 0),
    [rows]
  );
  ...
  function setRowAmount(idx: number, value: number) {
    setRows((prev) =>
      prev.map((r, i) =>
        i === idx ? { ...r, amount: Math.max(0, Math.floor(value || 0)) } : r
      )
    );
  }
  ...
  async function distribute() {
    ...
    try {
        ...
        setResult([
          {
            name: "Config",
            status: "error",
            error: "LNbits not configured in Settings → Account",
          },
        ]);
        setSubmitting(false);
        return;
      ...
      if (mode === "sats") {
        recipients = rows
          .filter((r) => r.enabled && (r.lud16 || r.lnurl) && r.amount > 0)
          .map((r) => ({
            lud16: r.lud16,
            lnurl: r.lnurl,
            amount: r.amount,
            comment: `${entity}/${repo} distribution`,
          }));
      } else {
          ...
          setResult([
            {
              name: "Validation",
              status: "error",
              error: "Percent splits must sum to 100",
            },
          ]);
          setSubmitting(false);
          return;
        ...
        const cap = parseInt(
          prompt(
            "Total sats to distribute (fees included if 'cap', else fees on top)"
          ) || "0"
        );
        if (!cap || cap <= 0) {
          setSubmitting(false);
        ...
        recipients = rows
          ...
          .map((r) => ({
            lud16: r.lud16,
            lnurl: r.lnurl,
            amount: Math.floor((cap * r.amount) / 100),
            comment: `${entity}/${repo} distribution`,
          }));
      }
      ...
      if (!resp.ok || data.error) {
        ...
        const rs = (data.results || []).map((r: any, idx: number) => ({
          name: rows[idx]?.name || `#${idx + 1}`,
          status: r.status,
          error: r.error,
        }));
        setResult(rs);
    ...
    <div className="mt-6">
      ...
      <div className="flex items-center gap-3 text-sm mb-3">
        ...
        <label className="flex items-center gap-1">
          <input
            type="radio"
            id="distribute-fee-gross"
            name="distribute-fee"
            checked={feeMode === "gross"}
            onChange={() => setFeeMode("gross")}
          />
          Fees on top
        </label>
        <label className="flex items-center gap-1">
          <input
            type="radio"
            id="distribute-fee-cap"
            name="distribute-fee"
            checked={feeMode === "cap"}
            onChange={() => setFeeMode("cap")}
          />
          Cap spend
        </label>
      </div>
        ...
        {rows.map((r, idx) => (
          <div
            ...
            className={`flex items-center gap-3 p-2 border rounded ${
              r.enabled ? "border-gray-700" : "border-gray-800 bg-gray-900/30"
            }`}
          >
            <label
              className="flex items-center gap-2 w-1/3"
              htmlFor={`distribute-checkbox-${idx}`}
            >
              <input
                type="checkbox"
                id={`distribute-checkbox-${idx}`}
                name={`distribute-enabled-${idx}`}
                checked={r.enabled}
                onChange={(e) =>
                  setRows((prev) =>
                    prev.map((x, i) =>
                ...
                disabled={!r.lud16 && !r.lnurl}
              />
              <span className="truncate" title={r.name}>
                {r.name}
              </span>
              {!r.lud16 && !r.lnurl && (
                <Badge className="ml-2">no lightning</Badge>
              )}
            </label>
            <input
              type="number"
              ...
              value={r.amount}
              onChange={(e) =>
                setRowAmount(idx, parseInt(e.target.value || "0"))
              }
      ...
      <div className="mt-3 text-sm text-gray-400">
        {mode === "sats" ? (
          <span>
            Total: {totalAmount} sats (fees{" "}
            {feeMode === "gross" ? "on top" : "included"})
          </span>
        ) : (
