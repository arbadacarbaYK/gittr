...
async function fetchGithubTree(
  ...
  try {
    ...
    const branchUrl = `https://api.github.com/repos/${owner}/${repo}/git/refs/heads/${encodeURIComponent(
      branch
    )}`;
    ...
    if (!r.ok) {
      ...
      console.error(
        `❌ [Import API] Failed to fetch tree: ${
          r.status
    ...
    if (!d.tree || !Array.isArray(d.tree)) {
      console.error(`❌ [Import API] Tree response invalid:`, {
        hasTree: !!d.tree,
        treeIsArray: Array.isArray(d.tree),
      });
      return null;
    }
    ...
    const dirs = d.tree
      ...
      .map((n: any) => ({ type: "dir", path: n.path as string }));
    ...
    console.error(
      `❌ [Import API] Error fetching tree for ${owner}/${repo}@${branch}:`,
      error.message
    );
    return null;
  }
...
async function fetchFileContent(
  ...
  try {
    ...
    const isBinary =
      !textExts.includes(ext) &&
      (binaryExts.includes(ext) || (data.size && data.size > 100 * 1024)); // Files > 100KB are likely binary (unless they're text files)
...
export default async function handler(
  ...
  if (!urlValidation.valid) {
    return res.status(400).json({
      status: "invalid_url",
      message: urlValidation.error || "Invalid GitHub URL",
    });
  }
  ...
  try {
    ...
    if (u.host !== "github.com")
      return res.status(400).json({ status: "only_github_supported_now" });
    ...
    if (!owner) {
      return res.status(400).json({
        status: "invalid_url",
        message: "Invalid GitHub URL: owner not found",
      });
    }
    ...
    const userToken =
      githubToken && typeof githubToken === "string" ? githubToken : null;
    ...
    if (tokenToUse) {
      ...
      console.log(
        `🔑 [Import API] Using ${
          userToken ? "user" : "platform"
        } GitHub token for ${owner}/${repo}`
      );
    }
    ...
    if (repoResponse.ok) {
      ...
      if (
        ...
        repoData.homepage.trim().length > 0
      ) {
        const rawHomepage = repoData.homepage.trim();
        // Ensure it has http:// or https://
        homepage =
          !rawHomepage.startsWith("http://") &&
          !rawHomepage.startsWith("https://")
            ? `https://${rawHomepage}`
            : rawHomepage;
      }
    }
      ...
      pulls = list
        .map((pr: any) => ({
          ...
          user: pr.user
            ...
            : undefined,
          labels: pr.labels
            ? pr.labels.map((l: any) => ({ name: l.name, color: l.color }))
            : undefined,
    ...
    if (!tree || !tree.files || tree.files.length === 0) {
      console.error(
        `❌ [Import API] Failed to fetch tree for ${owner}/${repo}`,
        {
          branchToUse,
          treeIsNull: !tree,
    ...
    let readme = "";
    const rawMain = `https://raw.githubusercontent.com/${owner}/${repo}/${encodeURIComponent(
      branchToUse
    )}/README.md`;
    const rawMaster = `https://raw.githubusercontent.com/${owner}/${repo}/main/README.md`;
    const baseUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${encodeURIComponent(
      branchToUse
    )}`;
    ...
    if (tree?.files) {
      ...
      console.log(
        `📥 [Import API] Processing ${fileList.length} files (metadata only, no content to avoid 4MB limit)...`
      );
      fileList.forEach((file: any) => {
        ...
        const isBinary =
          !textExts.includes(ext) &&
          (file.size === undefined || file.size > 100000); // Assume binary if >100KB or unknown extension
    ...
    const finalFiles =
      filesWithMetadata.length > 0 ? filesWithMetadata : tree?.files || [];
    ...
    if (payloadBytes > MAX_PAYLOAD_BYTES) {
      ...
      return res.status(413).json({
        ...
        fileCount:
          responsePayload.files?.filter((f) => f.type === "file").length ?? 0,
