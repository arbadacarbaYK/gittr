...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!entity || !repo || typeof entity !== "string" || typeof repo !== "string") {
  ...
  try {
    ...
    if (ownerPubkey) {
      ...
      iconUrl = `${baseUrl}/api/nostr/repo/file-content?ownerPubkey=${encodeURIComponent(ownerPubkey)}&repo=${encodeURIComponent(cleanRepoName)}&path=logo.png&branch=main`;
    }
