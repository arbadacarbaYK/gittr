...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      const activatePromise = fetch(
        `${finalLnbitsUrl}/api/v1/extension/${withdrawExtId}/activate`,
        {
          method: "PUT",
          headers: {
      ...
      const activateResponse = await Promise.race([
        activatePromise,
        activateTimeout,
      ]);
      ...
      if (!activateResponse.ok && activateResponse.status !== 409) {
        ...
        console.warn(
          "Extension activation check failed (user may need to enable manually):",
          errorText
        );
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `Network error: Unable to connect to LNbits at ${finalLnbitsUrl}. Error: ${
            fetchError.cause?.code || fetchError.message
          }. Please check your LNbits URL and network connection.`
        );
      }

      // Check if extension is not enabled
      if (
        fetchError.message?.includes("not enabled") ||
        fetchError.message?.includes("404")
      ) {
        throw new Error(
          `Withdraw extension is not enabled. Please enable it in LNbits UI (Extensions → Withdraw) for your wallet.`
        );
      }
    ...
    if (!withdrawId) {
      ...
      try {
        ...
        console.warn(
          "Failed to list withdraw links to get ID:",
          listError.message
        );
        return res.status(200).json({
          status: "ok",
      ...
      const createdLink = allLinks.find(
        (link: any) => link.title === bountyTitle
      );
