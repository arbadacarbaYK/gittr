...
import { validatePaymentAmount, validateTextContent, validateUrl } from "@/lib/security/input-validation";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { amount, issueId, description, lnbitsUrl, lnbitsAdminKey } = req.body || {};
  ...
  if (description) {
    const descValidation = validateTextContent(description, 1000, "Description");
    if (!descValidation.valid) {
      return res.status(400).json({ status: "invalid_description", message: descValidation.error });
    }
  }
