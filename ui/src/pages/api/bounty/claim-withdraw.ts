...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (recipientLud16) {
      ...
      const amount = linkData.max_withdrawable || linkData.min_withdrawable;
      const callbackResponse = await fetch(`${recipientLnurl}?amount=${amount}`);
      if (!callbackResponse.ok) {
    ...
    const claimResponse = await fetch(`${withdrawCallbackUrl}?k1=${k1}&pr=${encodeURIComponent(invoice)}`);
