...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (githubMatch) {
      ...
      const rawUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${encodeURIComponent(branchStr)}/${encodeURIComponent(filePathStr)}`;
      ...
      try {
        ...
        if (platformToken) {
          ...
          console.log(`🔍 [Git API] Using GitHub API (authenticated) instead of raw URL: ${apiUrl}`);
            ...
            if (jsonResponse.ok) {
              ...
              if (fileData.content && fileData.encoding === "base64") {
                ...
                const textFileExts = [
                  ...
                  'js', 'ts', 'jsx', 'tsx', 'py', 'java', 'c', 'cpp', 'h', 'hpp', 'cs', 'php', 'rb', 'go', 'rs',
                  'swift', 'kt', 'scala', 'clj', 'sh', 'bash', 'zsh', 'fish', 'r', 'm', 'mm', 'dart', 'lua',
                  'pl', 'pm', 'sql', 'hs', 'elm', 'ex', 'exs', 'erl', 'ml', 'mli', 'fs', 'fsx', 'vb', 'vim', 'vimrc',
                  'html', 'htm', 'xhtml', 'css', 'scss', 'sass', 'less', 'styl', 'vue', 'svelte',
                  'graphql', 'gql', 'prisma', 'psql', 'mysql', 'log', 'ini', 'conf', 'config', 'cfg',
                  'properties', 'env', 'gitignore', 'gitattributes', 'dockerignore', 'editorconfig', 'lock',
                  'csv', 'tsv', 'diff', 'patch', 'makefile', 'cmake', 'gradle', 'maven', 'pom'
                ];
                ...
                const binaryFileExts = [
                  // Images
                  'png', 'jpg', 'jpeg', 'gif', 'webp', 'svg', 'bmp', 'ico', 'avif', 'tiff', 'tif', 'heic', 'heif', 'apng', 'jfif', 'jp2', 'jpx', 'j2k',
                  // Videos
                  'mp4', 'webm', 'ogg', 'ogv', 'mov', 'avi', 'mkv', 'flv', 'wmv', 'm4v', '3gp', '3g2', 'asf', 'rm', 'rmvb', 'vob', 'mpg', 'mpeg', 'm2v',
                  // Audio
                  'mp3', 'wav', 'flac', 'aac', 'm4a', 'wma', 'opus', 'amr', 'au', 'ra', 'mid', 'midi', 'aiff', 'aif', 'caf',
                  // Documents
                  'pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'odt', 'ods', 'odp',
                  // Fonts
                  'woff', 'woff2', 'ttf', 'otf', 'eot', 'ttc',
                  // Archives & Compressed
                  'zip', 'tar', 'gz', 'bz2', 'xz', '7z', 'rar', 'dmg', 'iso', 'deb', 'rpm', 'pkg', 'cab', 'ar', 'cpio', 'shar', 'lz', 'lzma', 'lzo', 'z', 'zst', 'zstd',
                  // Installers & Executables (Release Assets)
                  'exe', 'dll', 'so', 'dylib', 'bin', 'app', 'apk', 'ipa', 'msi', 'msix', 'appx', 'appxbundle', 'snap',
                  // Platform-specific packages
                  'deb', 'rpm', 'pkg', 'dmg', 'pkg', 'flatpak', 'appimage',
                  // Other binaries
                  'wasm', 'o', 'obj', 'lib', 'a', 'jar', 'war', 'ear', 'class'
                ];
                ...
                if (isKnownText) {
                  try {
                    ...
                    console.log(`✅ [Git API] Successfully fetched text file from GitHub API: ${filePath} (${textContent.length} chars)`);
                    return res.status(200).json({
                      content: textContent,
                      isBinary: false,
                      path: filePath,
                    ...
                    console.warn(`⚠️ [Git API] Failed to decode base64 content, treating as binary:`, decodeError.message);
                    return res.status(200).json({
                ...
                try {
                  ...
                  console.log(`✅ [Git API] Successfully fetched file (unknown type, decoded as text) from GitHub API: ${filePath} (${textContent.length} chars)`);
                  return res.status(200).json({
                    content: textContent,
                    isBinary: false,
                    path: filePath,
                  ...
                  console.log(`⚠️ [Git API] Failed to decode file, treating as binary: ${filePath}`);
                  return res.status(200).json({
                    content: fileData.content,
                    isBinary: true,
                    path: filePath,
                ...
                console.log(`⚠️ [Git API] File uses Git LFS or is too large, falling back to raw URL`);
              }
            } else if (jsonResponse.status === 404) {
              console.log(`⚠️ [Git API] File not found on GitHub API: ${filePath}`);
              ...
              const retryAfter = jsonResponse.headers.get("retry-after") || "60";
              console.log(`⚠️ [Git API] GitHub API rate limit hit (429), retry after ${retryAfter}s`);
              return res.status(429).json({ 
                error: "GitHub rate limit exceeded",
                status: 429,
                retryAfter: parseInt(retryAfter, 10),
                message: "Too many requests to GitHub. Please try again in a few minutes.",
                path: filePath,
                branch,
              });
            } else {
              // API failed, fall through to try raw URL
              console.log(`⚠️ [Git API] GitHub JSON API failed (${jsonResponse.status}), trying raw URL as fallback...`);
            }
          } catch (apiError: any) {
            console.warn(`⚠️ [Git API] GitHub API request failed, trying raw URL:`, apiError.message);
        ...
        if (rawResponse.ok) {
          ...
          console.log(`✅ [Git API] Successfully fetched file from GitHub: ${filePath} (${txt.length} chars)`);
          return res.status(200).json({
            content: txt,
            isBinary: false,
            path: filePath,
          ...
          console.log(`⚠️ [Git API] GitHub rate limit hit (429), retry after ${retryAfter}s`);
          return res.status(429).json({ 
            error: "GitHub rate limit exceeded",
            status: 429,
            retryAfter: parseInt(retryAfter, 10),
          ...
          console.log(`⚠️ [Git API] GitHub raw URL failed (${rawResponse.status}), trying API...`);
          const endpoint = `/repos/${owner}/${repo}/contents/${encodeURIComponent(filePathStr)}?ref=${encodeURIComponent(branchStr)}`;
          const proxyUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/github/proxy?endpoint=${encodeURIComponent(endpoint)}`;
          ...
          try {
            ...
            console.error(`❌ [Git API] GitHub API proxy also failed:`, apiError.message);
          }
          ...
          console.error(`❌ [Git API] GitHub fetch failed: ${rawResponse.status} - ${errorText.substring(0, 200)}`);
        ...
        console.error(`❌ [Git API] Error fetching from GitHub:`, fetchError.message || fetchError);
      ...
      const apiUrl = `https://gitlab.com/api/v4/projects/${projectPath}/repository/files/${encodedFilePath}/raw?ref=${encodeURIComponent(branchStr)}`;
      ...
      const rawUrl = `https://gitlab.com/${projectPath}/-/raw/${encodeURIComponent(branchStr)}/${encodeURIComponent(filePathStr)}`;
      ...
      if (!response.ok) {
        ...
        console.log(`⚠️ [Git API] GitLab raw URL failed (${response.status}), trying API...`);
        ...
        if (apiResponse.ok) {
          ...
          console.error(`❌ [Git API] Both GitLab raw URL and API failed. Raw: ${response.status}, API: ${apiResponse.status}`);
          return res.status(response.status).json({ 
            error: "Failed to fetch file from GitLab",
            status: response.status,
            details: errorText.substring(0, 200),
            suggestion: response.status === 403 ? "This might be a rate limit or access restriction. The repository might require authentication." : undefined,
          });
        }
      }
      ...
      if (response.ok) {
        ...
        const errorText = await response.text().catch(() => "");
        console.error(`❌ [Git API] GitLab fetch failed: ${response.status} - ${errorText.substring(0, 200)}`);
        return res.status(response.status).json({ 
          error: "Failed to fetch file from GitLab",
          status: response.status,
          details: errorText.substring(0, 200),
      ...
      const rawUrl = `https://codeberg.org/${owner}/${repo}/raw/branch/${encodeURIComponent(branchStr)}/${encodeURIComponent(filePathStr)}`;
      ...
      try {
        ...
        if (response.ok) {
          ...
          const txt = await response.text();
          console.log(`✅ [Git API] Successfully fetched file from Codeberg: ${filePath} (${txt.length} chars)`);
          return res.status(200).json({
            content: txt,
            isBinary: false,
            path: filePath,
          ...
          const errorText = await response.text().catch(() => "");
          console.error(`❌ [Git API] Codeberg fetch failed: ${response.status} - ${errorText.substring(0, 200)}`);
          return res.status(response.status).json({ 
        ...
        console.error(`❌ [Git API] Error fetching from Codeberg:`, fetchError.message);
        return res.status(500).json({ 
          error: "Failed to fetch file from Codeberg",
