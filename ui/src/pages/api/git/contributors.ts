...
export default async function handler(
  ...
  try {
    ...
    if (githubMatch) {
      ...
      const contributorsUrl = `https://codeberg.org/api/v1/repos/${encodeURIComponent(
        owner
      ...
      return res.status(400).json({
        error:
          "Unsupported git service. Only GitHub, GitLab, and Codeberg are supported.",
      });
    }
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to fetch contributors" });
  }
}
