...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!entityStr) {
    return res.status(400).json({ error: "Entity (NIP-05 or npub) is required" });
  }
  ...
  if (entityStr.startsWith("npub")) {
    ...
    console.log(`✅ [NIP-05 Resolve] Already npub format, redirecting to: ${redirectUrl}`);
  ...
  if (/^[0-9a-f]{64}$/i.test(entityStr)) {
    ...
    try {
      ...
      console.log(`✅ [NIP-05 Resolve] Encoded hex to npub, redirecting to: ${redirectUrl}`);
      return res.redirect(301, redirectUrl);
    } catch (error: any) {
      return res.status(400).json({ error: `Failed to encode pubkey to npub: ${error?.message}` });
    }
  }
  ...
  if (entityStr.includes("@")) {
    try {
      ...
      return res.status(404).json({ error: `NIP-05 ${entityStr} did not return a valid pubkey` });
    } catch (error: any) {
      return res.status(500).json({ error: `Failed to resolve NIP-05 ${entityStr}: ${error?.message || "unknown error"}` });
    }
  }

  return res.status(400).json({ error: `Invalid entity format: must be npub, hex pubkey, or NIP-05 (received: ${entityStr})` });
}
