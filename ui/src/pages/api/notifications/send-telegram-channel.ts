...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!title || !message) {
    return res.status(400).json({ error: "missing_params", message: "Missing title or message" });
  }
  ...
  if (!telegramChatId) {
    ...
    return res.status(500).json({ error: "not_configured", message: "Telegram channel ID not configured" });
  }
  ...
  try {
    ...
    if (!telegramResponse.ok) {
      ...
      return res.status(500).json({ 
        error: "send_failed", 
        message: errorData.description || "Failed to send Telegram channel message" 
      });
    }
