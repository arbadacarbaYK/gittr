...
export default async function handler(
  ...
  try {
    if (splits && splits.length > 0) {
      ...
      console.log(
        "LNbits Split: Creating invoice from source wallet for auto-split"
      );
      console.log(
        "Split info (for reference only):",
        splits.map((s: any) => ({ pubkey: s.pubkey, weight: s.weight + "%" }))
      );
      ...
      if (!fullInvoice || fullInvoice.length < 50) {
        return res.status(500).json({
          status: "error",
          message:
            "Invalid invoice received from LNbits (too short or missing)",
        });
      }
      ...
      if (!lud16 && !lnurl) {
        return res.status(400).json({
          ...
          message:
            "Recipient's Lightning address (lud16 or lnurl) is required. Please fetch from Nostr profile first.",
        });
      }
      ...
      if (lud16) {
        console.log(
          "LNbits Zap: Creating invoice from recipient LUD-16:",
          lud16
        );
        ...
        console.log(
          "LNbits Zap: Creating invoice from recipient LNURL:",
          lnurl
        );
      ...
      console.log(
        "LNbits Zap: Invoice created from recipient:",
      ...
      const paymentRequest: LNbitsPaymentRequest = {
        ...
        bolt11: invoice,
        memo:
          comment || `Zap: ${amountNum} sats to ${recipient.slice(0, 8)}...`,
        extra: {
          comment:
            comment || `Zap: ${amountNum} sats to ${recipient.slice(0, 8)}...`,
          recipient,
