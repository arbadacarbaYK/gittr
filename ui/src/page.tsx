...
export default function EntityPage({ params }: { params: { entity: string } }) {
  ...
  const [fullPubkeyForMeta, setFullPubkeyForMeta] = useState<string>(() => {
    ...
    if (params.entity.length === 64 && /^[0-9a-f]{64}$/i.test(params.entity)) {
      console.log(
        `✅ [Profile] Entity is full pubkey: ${params.entity.slice(0, 8)}`
      );
    ...
    try {
      ...
      const matchingRepo = repos.find(
        (r: any) =>
          ...
          (params.entity.length === 8 &&
            r.entity?.toLowerCase().startsWith(params.entity.toLowerCase())) ||
          (r.ownerPubkey &&
            /^[0-9a-f]{64}$/i.test(r.ownerPubkey) &&
            r.ownerPubkey.toLowerCase().startsWith(params.entity.toLowerCase()))
      );
      ...
      if (
        ...
        console.log(
          `✅ [Profile] Resolved pubkey from repo: ${matchingRepo.ownerPubkey.slice(
            0,
            8
          )}`
        );
      ...
      const matchingActivity = activities.find(
        (a: any) =>
          ...
          a.user.toLowerCase().startsWith(params.entity.toLowerCase())
      );
      if (matchingActivity?.user) {
        console.log(
          `✅ [Profile] Resolved pubkey from activity: ${matchingActivity.user.slice(
            0,
            8
          )}`
        );
    ...
    console.log(
      `⚠️ [Profile] Could not resolve full pubkey for entity: ${params.entity}`
    );
    return params.entity;
  });
    ...
    if (params.entity && /^[0-9a-f]{8}$/i.test(params.entity)) {
      try {
        ...
        const matchingRepo = repos.find(
          (r: any) =>
            (r.ownerPubkey &&
              ...
              r.ownerPubkey
                .toLowerCase()
                .startsWith(params.entity.toLowerCase())) ||
            (r.entity &&
        ...
        if (
          ...
          console.log(
            `📡 [Profile] Resolved pubkey from repo (useMemo): ${matchingRepo.ownerPubkey.slice(
              0,
              8
            )}`
          );
        ...
        if (matchingActivity?.user) {
          console.log(
            `📡 [Profile] Resolved pubkey from activity (useMemo): ${matchingActivity.user.slice(
              0,
              8
            )}`
          );
        ...
        console.error(
          `❌ [Profile] Failed to resolve from localStorage in useMemo:`,
          e
        );
      }
    }
    ...
    if (currentUserPubkey && /^[0-9a-f]{64}$/i.test(currentUserPubkey)) {
      ...
      if (resolvedPubkey && /^[0-9a-f]{64}$/i.test(resolvedPubkey)) {
        ...
        if (params.entity.startsWith("npub")) {
          ...
          params.entity &&
          /^[0-9a-f]{64}$/i.test(params.entity) &&
          params.entity.toLowerCase() === normalizedCurrentPubkey
        ) {
          console.log(
    ...
    if (pubkeysArray.length === 0) {
      console.log(
        `⏭️ [Profile] Skipping metadata fetch - no valid pubkey. Entity: ${
          ...
          params.entity.length
        } chars), fullPubkeyForMeta: ${fullPubkeyForMeta} (${
          fullPubkeyForMeta?.length || 0
        } chars)`
      );
    }
    return pubkeysArray;
  ...
  useEffect(() => {
    if (pubkeyForMetadata) {
      ...
      console.log(
        `⚠️ [Profile] No pubkey for metadata! Entity: ${params.entity}, fullPubkeyForMeta: ${fullPubkeyForMeta}`
      );
    }
  }, [
    userMeta,
      ...
      if (isFullPubkey || isEntityPrefix) {
        ...
        if (missingRepos.length > 0) {
          ...
          if (syncedRepos.length > 0) {
            ...
            const filteredUpdatedRepos = updatedUserReposList.filter(
              (r: any) => {
        ...
        console.log("Profile stats for", fullPubkey, {
          ...
          bountiesClaimed: activities.filter((a) => a.type === "bounty_claimed")
            .length,
          counts: counts,
    ...
    if (
      ...
      if (isValidPubkey(params.entity) && params.entity.length === 64) {
        if (fullPubkeyForMeta !== params.entity) {
          console.log(
            `✅ [Profile] useEffect: Setting fullPubkeyForMeta from entity (full): ${params.entity.slice(
              0,
              8
            )}`
          );
      ...
      if (params.entity && /^[0-9a-f]{8}$/i.test(params.entity)) {
        try {
          ...
          if (matchingRepo?.entity) {
            ...
            if (
              matchingActivity?.user &&
              fullPubkeyForMeta !== matchingActivity.user
            ) {
              console.log(
  ...
  const displayName = useMemo(() => {
    ...
    return params.entity.length === 8
      ? params.entity
      : params.entity.slice(0, 8);
  ...
  const picture =
    userMeta?.picture ||
    (fullPubkeyForMeta && /^[0-9a-f]{64}$/i.test(fullPubkeyForMeta)
      ? getUserMetadata(fullPubkeyForMeta.toLowerCase(), metadataMap)?.picture
      : null) ||
        ...
        if (event.kind === 3) {
          try {
            ...
            if (
              ...
              const pubkeys = contacts.p.map((p: any) =>
  ...
  const handleFollow = async () => {
    // Legacy unused profile page (not routed under app/). Refuse publishes —
    // safe Follow lives in app/[entity]/page.tsx only.
    alert(
      "Follow from this legacy page is disabled. Open the person’s profile URL and use Follow there."
    );
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {userRepos.map((repo: any) => {
              ...
              if (!repo.entity || !repoForUrl) {
                console.error("⚠️ [Profile] Invalid repo data for link:", {
                  entity: repo.entity,
                  repo: repoForUrl,
                  name: repo.name,
                  slug: repo.slug,
                ...
                return (
                  <div
                    key={`invalid-${repo.slug || repo.name || Math.random()}`}
                    className="hidden"
                  />
                );
              }
              ...
              try {
                ...
                if (ownerPubkey && /^[0-9a-f]{64}$/i.test(ownerPubkey)) {
                  ...
                  const ownerMeta =
                    metadataMap[normalizedOwnerPubkey] ||
                  ...
                  if (ownerMeta?.picture && !iconUrl) {
                    ...
                    if (
                      fallbackMeta?.picture &&
                      fallbackMeta.picture !== ownerMeta?.picture
                    ) {
                      console.warn(
                        `⚠️ [Profile] getUserMetadata found different picture for ${normalizedOwnerPubkey.slice(
                          0,
                          8
                        )}... (might be wrong user due to partial match)`
                      );
                    }
                    console.log(
                      `⚠️ [Profile] No icon found for repo ${
                        repo.name || repo.slug
                      }, ownerPubkey: ${normalizedOwnerPubkey.slice(
                        0,
                        8
                      )}..., hasMetadata: ${!!ownerMeta}, hasPicture: ${!!ownerMeta?.picture}, metadataKeys: ${Object.keys(
                        metadataMap
                      )
                        .filter((k) =>
                          k
                            .toLowerCase()
                            .startsWith(normalizedOwnerPubkey.slice(0, 8))
                        )
                        .join(", ")}`
                    );
                  }
                  ...
                  console.log(
                    `⚠️ [Profile] No valid ownerPubkey for repo ${
                      repo.name || repo.slug
                    }, ownerPubkey: ${
                      ownerPubkey || "null"
                    }, repo.ownerPubkey: ${
                      repo.ownerPubkey?.slice(0, 8) || "null"
                    }, getRepoOwnerPubkey: ${
                      getRepoOwnerPubkey(repo, repo.entity)?.slice(0, 8) ||
                      "null"
                    }`
                  );
                }
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h3 className="font-semibold text-purple-400 break-words">
                          {repoDisplayName}
                        </h3>
                        {repo.userRole && (
                          <span
