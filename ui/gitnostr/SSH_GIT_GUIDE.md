...
**SSH vs gittr Code tab:** SSH/HTTPS serve this bridge’s `repositoryDir`. The website uses the latest live **30617**; a forge **`source`** is the Code-tab tree when present (this disk can be behind until Push `sync-from-source`); otherwise first non-empty `clone[]` listing. See gittr [`FILE_FETCHING_INSIGHTS.md`](https://github.com/arbadacarbaYK/gittr/blob/main/docs/FILE_FETCHING_INSIGHTS.md). Local twin: [`docs/file-fetch-flow.md`](docs/file-fetch-flow.md).
...
1. **Website:** repo page → **Push to Nostr**. Approve the signature however you logged in:
   ...
   - **NIP-46 remote signer** (Amber on your phone, bunker, …), or
...
- Announcing publishes NIP-34 events to Nostr relays
- Both are needed for full functionality: bridge for git operations, Nostr events for discovery
- Publishing to Nostr always requires **a signer**. On gittr.space that is **NIP-07**, **NIP-46 remote (Amber / bunker)**, or nsec. MCP/`gn` use a local `nsec`.
...
- ⚠️ **nsec in the browser**: If you must, use a dedicated browser profile — MCP/`gn` can keep the nsec in a keys file instead
