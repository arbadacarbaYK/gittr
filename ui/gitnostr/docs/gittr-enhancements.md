...
| 🆕 Watch-all mode | If `gitRepoOwners` is empty in the config, the bridge monitors **all** authors’ NIP-34 events (ACL/metadata). **Bare disk** is created only when `clone[]` includes this GRASP host (`git.gittr.space`) — not a full mirror of every GitHub/ngit announce. | Public GRASP hosting without storing the whole Nostr-git network. |
