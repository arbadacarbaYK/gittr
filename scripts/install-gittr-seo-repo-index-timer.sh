#!/usr/bin/env bash
# Install daily systemd timer that triggers GET /api/seo/refresh-nostr-repo-index?refresh=1
#
...
rm -f /tmp/gittr-seo-repo-index-refresh.service /tmp/gittr-seo-repo-index-refresh.timer
systemctl daemon-reload
systemctl enable --now gittr-seo-repo-index-refresh.timer
systemctl start gittr-seo-repo-index-refresh.service || true
echo ""
